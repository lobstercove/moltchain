...
use moltchain_core::{
    contract::ContractAccount, Account, Pubkey, StateStore,
    SymbolRegistryEntry, CONTRACT_PROGRAM_ID,
};
...
async fn rpc_p(
    ...
    let response = app.clone().oneshot(request).await.map_err(|e| format!("response error: {e}"))?;
    let body = to_bytes(response.into_body(), usize::MAX).await.map_err(|e| format!("body error: {e}"))?;
...
fn app_with_state() -> (axum::Router, String) {
    ...
    contract.storage.insert(b"test_key".to_vec(), b"test_value".to_vec());
    ...
    state.put_account(&contract_prog, &contract_acct).expect("put contract");

    // Register symbol
    state.register_symbol("TST", SymbolRegistryEntry {
...
async fn test_native_get_account_nonexistent() {
    let app = fresh_app();
    let resp = rpc_p(&app, "/", "getAccount", json!(["11111111111111111111111111111111"])).await.unwrap();
...
async fn test_native_get_account_existing() {
    ...
    if let Some(result) = resp.get("result") {
        if !result.is_null() {
            assert!(result.get("shells").is_some() || result.get("balance").is_some() || result.get("lamports").is_some(),
...
async fn test_native_get_transaction_nonexistent() {
    ...
    let resp = rpc_p(&app, "/", "getTransaction", json!([fake_sig])).await.unwrap();
    assert_valid_rpc(&resp);
}
...
async fn test_native_get_transactions_by_address() {
    let app = fresh_app();
    let resp = rpc_p(&app, "/", "getTransactionsByAddress",
        json!(["11111111111111111111111111111111"])).await.unwrap();
    assert_valid_rpc(&resp);
}
...
async fn test_native_get_account_tx_count() {
    let app = fresh_app();
    let resp = rpc_p(&app, "/", "getAccountTxCount",
        json!(["11111111111111111111111111111111"])).await.unwrap();
    assert_valid_rpc(&resp);
}
...
async fn test_native_get_recent_transactions() {
    let app = fresh_app();
    let resp = rpc_p(&app, "/", "getRecentTransactions", json!([10])).await.unwrap();
    assert_valid_rpc(&resp);
}
...
async fn test_native_get_token_accounts() {
    let app = fresh_app();
    let resp = rpc_p(&app, "/", "getTokenAccounts",
        json!(["11111111111111111111111111111111"])).await.unwrap();
    assert_valid_rpc(&resp);
}
...
async fn test_native_set_fee_config_no_token() {
    ...
    let app = fresh_app();
    let resp = rpc_p(&app, "/", "setFeeConfig", json!([{"base_fee": 100}])).await.unwrap();
    assert_valid_rpc(&resp);
    // Should error because no admin token configured
    assert!(resp.get("error").is_some(), "setFeeConfig without admin token should error");
}
...
async fn test_native_set_rent_params_no_token() {
    let app = fresh_app();
    let resp = rpc_p(&app, "/", "setRentParams", json!([{"exempt_minimum": 100}])).await.unwrap();
    assert_valid_rpc(&resp);
    assert!(resp.get("error").is_some(), "setRentParams without admin token should error");
}
...
async fn test_native_get_validator_info() {
    let app = fresh_app();
    let resp = rpc_p(&app, "/", "getValidatorInfo",
        json!(["11111111111111111111111111111111"])).await.unwrap();
    assert_valid_rpc(&resp);
}
...
async fn test_native_get_validator_performance() {
    let app = fresh_app();
    let resp = rpc_p(&app, "/", "getValidatorPerformance",
        json!(["11111111111111111111111111111111"])).await.unwrap();
    assert_valid_rpc(&resp);
}
...
async fn test_native_get_staking_status() {
    let app = fresh_app();
    let resp = rpc_p(&app, "/", "getStakingStatus",
        json!(["11111111111111111111111111111111"])).await.unwrap();
    assert_valid_rpc(&resp);
}
...
async fn test_native_get_staking_rewards() {
    let app = fresh_app();
    let resp = rpc_p(&app, "/", "getStakingRewards",
        json!(["11111111111111111111111111111111"])).await.unwrap();
    assert_valid_rpc(&resp);
}
...
async fn test_native_get_staking_position() {
    let app = fresh_app();
    let resp = rpc_p(&app, "/", "getStakingPosition",
        json!(["11111111111111111111111111111111"])).await.unwrap();
    assert_valid_rpc(&resp);
}
...
async fn test_native_get_unstaking_queue() {
    let app = fresh_app();
    let resp = rpc_p(&app, "/", "getUnstakingQueue",
        json!(["11111111111111111111111111111111"])).await.unwrap();
    assert_valid_rpc(&resp);
}
...
async fn test_native_get_account_info() {
    let (app, addr) = app_with_state();
    let resp = rpc_p(&app, "/", "getAccountInfo", json!([addr])).await.unwrap();
    assert_valid_rpc(&resp);
}
...
async fn test_native_get_transaction_history() {
    let app = fresh_app();
    let resp = rpc_p(&app, "/", "getTransactionHistory",
        json!(["11111111111111111111111111111111"])).await.unwrap();
    assert_valid_rpc(&resp);
}
...
async fn test_native_get_contract_info() {
    let app = fresh_app();
    let resp = rpc_p(&app, "/", "getContractInfo",
        json!(["11111111111111111111111111111111"])).await.unwrap();
    assert_valid_rpc(&resp);
}
...
async fn test_native_get_contract_logs() {
    let app = fresh_app();
    let resp = rpc_p(&app, "/", "getContractLogs",
        json!(["11111111111111111111111111111111"])).await.unwrap();
    assert_valid_rpc(&resp);
}
...
async fn test_native_get_contract_abi() {
    let app = fresh_app();
    let resp = rpc_p(&app, "/", "getContractAbi",
        json!(["11111111111111111111111111111111"])).await.unwrap();
    assert_valid_rpc(&resp);
}
...
async fn test_native_set_contract_abi_no_token() {
    let app = fresh_app();
    let resp = rpc_p(&app, "/", "setContractAbi",
        json!(["11111111111111111111111111111111", []])).await.unwrap();
    assert_valid_rpc(&resp);
    assert!(resp.get("error").is_some(), "setContractAbi without token should error");
}
...
async fn test_native_deploy_contract_missing_tx() {
    ...
    let resp = rpc_p(&app, "/", "deployContract", json!(["invalid"])).await.unwrap();
    assert_valid_rpc(&resp);
    assert!(resp.get("error").is_some(), "deployContract with invalid data should error");
}

#[tokio::test]
async fn test_native_upgrade_contract_missing_tx() {
    let app = fresh_app();
    let resp = rpc_p(&app, "/", "upgradeContract", json!(["invalid"])).await.unwrap();
    assert_valid_rpc(&resp);
    assert!(resp.get("error").is_some(), "upgradeContract with invalid data should error");
}
...
async fn test_native_get_program_stats() {
    let app = fresh_app();
    let resp = rpc_p(&app, "/", "getProgramStats",
        json!(["11111111111111111111111111111111"])).await.unwrap();
    assert_valid_rpc(&resp);
}
...
async fn test_native_get_program_calls() {
    let app = fresh_app();
    let resp = rpc_p(&app, "/", "getProgramCalls",
        json!(["11111111111111111111111111111111"])).await.unwrap();
    assert_valid_rpc(&resp);
}
...
async fn test_native_get_program_storage() {
    let app = fresh_app();
    let resp = rpc_p(&app, "/", "getProgramStorage",
        json!(["11111111111111111111111111111111"])).await.unwrap();
    assert_valid_rpc(&resp);
}
...
async fn test_native_get_evm_registration_no_mapping() {
    let app = fresh_app();
    let resp = rpc_p(&app, "/", "getEvmRegistration",
        json!(["11111111111111111111111111111111"])).await.unwrap();
    assert_valid_rpc(&resp);
...
async fn test_native_lookup_evm_address_no_mapping() {
    let app = fresh_app();
    let resp = rpc_p(&app, "/", "lookupEvmAddress",
        json!(["0x0000000000000000000000000000000000000001"])).await.unwrap();
    assert_valid_rpc(&resp);
    assert!(resp["result"].is_null(), "no mapping should return null");
}
...
async fn test_native_get_symbol_registry() {
    let (app, _) = app_with_state();
    let resp = rpc_p(&app, "/", "getSymbolRegistry", json!(["TST"])).await.unwrap();
    assert_valid_rpc(&resp);
    if let Some(result) = resp.get("result") {
        if !result.is_null() {
...
async fn test_native_get_symbol_registry_missing() {
    let app = fresh_app();
    let resp = rpc_p(&app, "/", "getSymbolRegistry", json!(["NOSYMBOL"])).await.unwrap();
    assert_valid_rpc(&resp);
}
...
async fn test_native_get_symbol_registry_by_program() {
    let app = fresh_app();
    let resp = rpc_p(&app, "/", "getSymbolRegistryByProgram",
        json!(["11111111111111111111111111111111"])).await.unwrap();
    assert_valid_rpc(&resp);
}
...
async fn test_native_get_all_symbol_registry() {
    let (app, _) = app_with_state();
    let resp = rpc_p(&app, "/", "getAllSymbolRegistry", json!([{}])).await.unwrap();
    assert_valid_rpc(&resp);
}
...
async fn test_native_get_collection() {
    let app = fresh_app();
    let resp = rpc_p(&app, "/", "getCollection",
        json!(["11111111111111111111111111111111"])).await.unwrap();
    assert_valid_rpc(&resp);
}
...
async fn test_native_get_nft() {
    let app = fresh_app();
    let resp = rpc_p(&app, "/", "getNFT",
        json!(["11111111111111111111111111111111", 0])).await.unwrap();
    assert_valid_rpc(&resp);
}
...
async fn test_native_get_nfts_by_owner() {
    let app = fresh_app();
    let resp = rpc_p(&app, "/", "getNFTsByOwner",
        json!(["11111111111111111111111111111111"])).await.unwrap();
    assert_valid_rpc(&resp);
}
...
async fn test_native_get_nfts_by_collection() {
    let app = fresh_app();
    let resp = rpc_p(&app, "/", "getNFTsByCollection",
        json!(["11111111111111111111111111111111"])).await.unwrap();
    assert_valid_rpc(&resp);
}
...
async fn test_native_get_nft_activity() {
    let app = fresh_app();
    let resp = rpc_p(&app, "/", "getNFTActivity",
        json!(["11111111111111111111111111111111"])).await.unwrap();
    assert_valid_rpc(&resp);
}
...
async fn test_native_get_market_listings() {
    let app = fresh_app();
    let resp = rpc_p(&app, "/", "getMarketListings", json!([{}])).await.unwrap();
    assert_valid_rpc(&resp);
}
...
async fn test_native_get_market_sales() {
    let app = fresh_app();
    let resp = rpc_p(&app, "/", "getMarketSales", json!([{}])).await.unwrap();
    assert_valid_rpc(&resp);
}
...
async fn test_native_get_token_balance() {
    let app = fresh_app();
    let resp = rpc_p(&app, "/", "getTokenBalance",
        json!(["11111111111111111111111111111111", "11111111111111111111111111111111"]))
        .await.unwrap();
    assert_valid_rpc(&resp);
}
...
async fn test_native_get_token_holders() {
    let app = fresh_app();
    let resp = rpc_p(&app, "/", "getTokenHolders",
        json!(["11111111111111111111111111111111"])).await.unwrap();
    assert_valid_rpc(&resp);
}
...
async fn test_native_get_token_transfers() {
    let app = fresh_app();
    let resp = rpc_p(&app, "/", "getTokenTransfers",
        json!(["11111111111111111111111111111111"])).await.unwrap();
    assert_valid_rpc(&resp);
}
...
async fn test_native_get_contract_events() {
    let app = fresh_app();
    let resp = rpc_p(&app, "/", "getContractEvents",
        json!(["11111111111111111111111111111111"])).await.unwrap();
    assert_valid_rpc(&resp);
}
...
async fn test_native_get_signatures_for_address() {
    let app = fresh_app();
    let resp = rpc_p(&app, "/", "getSignaturesForAddress",
        json!(["11111111111111111111111111111111"])).await.unwrap();
    assert_valid_rpc(&resp);
}
...
async fn test_native_get_prediction_markets() {
    let app = fresh_app();
    let resp = rpc_p(&app, "/", "getPredictionMarkets", json!([{}])).await.unwrap();
    assert_valid_rpc(&resp);
}
...
async fn test_native_get_prediction_market_by_id() {
    let app = fresh_app();
    let resp = rpc_p(&app, "/", "getPredictionMarket", json!([0])).await.unwrap();
    assert_valid_rpc(&resp);
}
...
async fn test_native_get_prediction_positions() {
    let app = fresh_app();
    let resp = rpc_p(&app, "/", "getPredictionPositions",
        json!(["11111111111111111111111111111111"])).await.unwrap();
    assert_valid_rpc(&resp);
}
...
async fn test_native_get_prediction_trader_stats() {
    let app = fresh_app();
    let resp = rpc_p(&app, "/", "getPredictionTraderStats",
        json!(["11111111111111111111111111111111"])).await.unwrap();
    assert_valid_rpc(&resp);
}
...
async fn test_native_get_prediction_leaderboard() {
    let app = fresh_app();
    let resp = rpc_p(&app, "/", "getPredictionLeaderboard", json!([{}])).await.unwrap();
    assert_valid_rpc(&resp);
}
...
async fn test_native_get_prediction_market_analytics() {
    let app = fresh_app();
    let resp = rpc_p(&app, "/", "getPredictionMarketAnalytics", json!([0])).await.unwrap();
    assert_valid_rpc(&resp);
}
...
async fn test_native_search_molt_names() {
    let app = fresh_app();
    let resp = rpc_p(&app, "/", "searchMoltNames", json!(["test"])).await.unwrap();
    assert_valid_rpc(&resp);
}
...
async fn test_native_request_airdrop() {
    let app = fresh_app();
    let resp = rpc_p(&app, "/", "requestAirdrop",
        json!(["11111111111111111111111111111111", 1000])).await.unwrap();
    assert_valid_rpc(&resp);
}
...
async fn test_native_send_transaction_no_sender() {
    ...
    let app = fresh_app();
    let resp = rpc_p(&app, "/", "sendTransaction", json!(["deadbeef"])).await.unwrap();
    assert_valid_rpc(&resp);
    assert!(resp.get("error").is_some(), "sendTransaction without tx_sender should error");
}
...
async fn test_native_confirm_transaction_nonexistent() {
    let app = fresh_app();
    let fake_sig = "a".repeat(64);
    let resp = rpc_p(&app, "/", "confirmTransaction", json!([fake_sig])).await.unwrap();
    assert_valid_rpc(&resp);
}
...
async fn test_native_simulate_transaction_invalid() {
    let app = fresh_app();
    let resp = rpc_p(&app, "/", "simulateTransaction", json!(["invalid_data"])).await.unwrap();
    assert_valid_rpc(&resp);
    // Should error because data is not a valid transaction
    assert!(resp.get("error").is_some(), "simulateTransaction with invalid data should error");
}
...
async fn test_native_stake_to_reefstake() {
    let app = fresh_app();
    let resp = rpc_p(&app, "/", "stakeToReefStake",
        json!(["11111111111111111111111111111111", 1000])).await.unwrap();
    assert_valid_rpc(&resp);
}
...
async fn test_native_unstake_from_reefstake() {
    let app = fresh_app();
    let resp = rpc_p(&app, "/", "unstakeFromReefStake",
        json!(["11111111111111111111111111111111", 1000])).await.unwrap();
    assert_valid_rpc(&resp);
}
...
async fn test_native_claim_unstaked_tokens() {
    let app = fresh_app();
    let resp = rpc_p(&app, "/", "claimUnstakedTokens",
        json!(["11111111111111111111111111111111"])).await.unwrap();
    assert_valid_rpc(&resp);
}
...
async fn test_solana_get_balance() {
    let app = fresh_app();
    let resp = rpc_p(&app, "/solana", "getBalance",
        json!(["11111111111111111111111111111111"])).await.unwrap();
    assert_valid_rpc(&resp);
}
...
async fn test_solana_get_account_info() {
    let app = fresh_app();
    let resp = rpc_p(&app, "/solana", "getAccountInfo",
        json!(["11111111111111111111111111111111"])).await.unwrap();
    assert_valid_rpc(&resp);
}
...
async fn test_solana_get_signatures_for_address() {
    let app = fresh_app();
    let resp = rpc_p(&app, "/solana", "getSignaturesForAddress",
        json!(["11111111111111111111111111111111"])).await.unwrap();
    assert_valid_rpc(&resp);
}
...
async fn test_solana_get_signature_statuses() {
    let app = fresh_app();
    let fake_sig = "a".repeat(64);
    let resp = rpc_p(&app, "/solana", "getSignatureStatuses",
        json!([[fake_sig]])).await.unwrap();
    assert_valid_rpc(&resp);
}
...
async fn test_solana_get_transaction() {
    let app = fresh_app();
    let fake_sig = "a".repeat(64);
    let resp = rpc_p(&app, "/solana", "getTransaction", json!([fake_sig])).await.unwrap();
    assert_valid_rpc(&resp);
}
...
async fn test_solana_send_transaction_no_sender() {
    let app = fresh_app();
    let resp = rpc_p(&app, "/solana", "sendTransaction", json!(["deadbeef"])).await.unwrap();
    assert_valid_rpc(&resp);
}
...
async fn test_evm_eth_get_balance() {
    let app = fresh_app();
    let resp = rpc_p(&app, "/evm", "eth_getBalance",
        json!(["0x0000000000000000000000000000000000000001", "latest"])).await.unwrap();
    assert_valid_rpc(&resp);
}
...
async fn test_evm_eth_send_raw_transaction() {
    let app = fresh_app();
    let resp = rpc_p(&app, "/evm", "eth_sendRawTransaction",
        json!(["0xdeadbeef"])).await.unwrap();
    assert_valid_rpc(&resp);
}
...
async fn test_evm_eth_call() {
    let app = fresh_app();
    let resp = rpc_p(&app, "/evm", "eth_call",
...
async fn test_evm_eth_get_transaction_receipt() {
    ...
    let resp = rpc_p(&app, "/evm", "eth_getTransactionReceipt",
        json!([fake_hash])).await.unwrap();
    assert_valid_rpc(&resp);
}
...
async fn test_evm_eth_get_transaction_by_hash() {
    let app = fresh_app();
    let fake_hash = format!("0x{}", "a".repeat(64));
    let resp = rpc_p(&app, "/evm", "eth_getTransactionByHash",
        json!([fake_hash])).await.unwrap();
    assert_valid_rpc(&resp);
}
...
async fn test_evm_eth_estimate_gas() {
    let app = fresh_app();
    let resp = rpc_p(&app, "/evm", "eth_estimateGas",
        json!([{"to": "0x0000000000000000000000000000000000000001"}])).await.unwrap();
    assert_valid_rpc(&resp);
}
...
async fn test_evm_eth_get_code() {
    let app = fresh_app();
    let resp = rpc_p(&app, "/evm", "eth_getCode",
        json!(["0x0000000000000000000000000000000000000001", "latest"])).await.unwrap();
    assert_valid_rpc(&resp);
}
...
async fn test_evm_eth_get_transaction_count() {
    let app = fresh_app();
    let resp = rpc_p(&app, "/evm", "eth_getTransactionCount",
        json!(["0x0000000000000000000000000000000000000001", "latest"])).await.unwrap();
    assert_valid_rpc(&resp);
}
...
async fn test_evm_eth_get_block_by_number() {
    let app = fresh_app();
    let resp = rpc_p(&app, "/evm", "eth_getBlockByNumber",
        json!(["0x0", false])).await.unwrap();
    assert_valid_rpc(&resp);
}
...
async fn test_evm_eth_get_block_by_hash() {
    ...
    let resp = rpc_p(&app, "/evm", "eth_getBlockByHash",
        json!([fake_hash, false])).await.unwrap();
    assert_valid_rpc(&resp);
}
...
async fn test_evm_eth_get_storage_at() {
    let app = fresh_app();
    let resp = rpc_p(&app, "/evm", "eth_getStorageAt",
        json!(["0x0000000000000000000000000000000000000001", "0x0", "latest"])).await.unwrap();
    assert_valid_rpc(&resp);
}
...
async fn test_evm_web3_client_version() {
    ...
    assert!(ver.starts_with("MoltChain/"), "should start with MoltChain/: {ver}");
}
...
async fn test_native_missing_params_get_balance() {
    ...
    assert!(resp.get("error").is_some(), "getBalance without params should error");
}
...
async fn test_native_get_block_very_high_slot() {
    let app = fresh_app();
    let resp = rpc_p(&app, "/", "getBlock", json!([999999999])).await.unwrap();
    assert_valid_rpc(&resp);
}
...
async fn test_evm_invalid_address_format() {
    let app = fresh_app();
    let resp = rpc_p(&app, "/evm", "eth_getBalance",
        json!(["not-an-address", "latest"])).await.unwrap();
    assert_valid_rpc(&resp);
    assert!(resp.get("error").is_some());
}
...
async fn test_all_stats_endpoints_return_valid_json() {
    ...
    let stats_methods = vec![
        ...
        "getMoltPunksStats", "getPredictionMarketStats",
    ];
...
async fn test_all_evm_methods_return_valid_jsonrpc() {
    let app = fresh_app();
    let methods = vec![
        "eth_chainId", "eth_blockNumber", "eth_accounts",
        "net_version", "eth_gasPrice", "eth_maxPriorityFeePerGas",
