...
const ACHIEVEMENT_DEFS = [
    { id: 1, name: 'First Transaction', icon: 'fas fa-exchange-alt', desc: 'Complete your first on-chain transaction' },
    { id: 2, name: 'Governance Voter', icon: 'fas fa-vote-yea', desc: 'Cast a governance vote' },
    { id: 3, name: 'Builder', icon: 'fas fa-code', desc: 'Deploy a smart contract' },
    { id: 4, name: 'Trusted', icon: 'fas fa-shield-alt', desc: 'Reach 500+ reputation' },
    { id: 5, name: 'Veteran', icon: 'fas fa-medal', desc: 'Reach 1,000+ reputation' },
    { id: 6, name: 'Legend', icon: 'fas fa-crown', desc: 'Reach 5,000+ reputation' },
    { id: 7, name: 'Endorsed', icon: 'fas fa-handshake', desc: 'Receive 10+ vouches' },
    { id: 8, name: 'Graduated', icon: 'fas fa-graduation-cap', desc: 'Complete identity verification' },
];
...
function renderAchievementsSection(achievements, achievedIds) {
    const all = ACHIEVEMENT_DEFS.map(def => {
        ...
        return `<span class="id-badge ${earned ? 'id-badge-earned' : 'id-badge-locked'}" title="${escHtml(def.desc)}"><i class="${def.icon}"></i> ${def.name}</span>`;
    }).join('');
...
// ============================================================================
// PASSWORD MODAL ENHANCEMENT — Support for <select> fields
// ============================================================================
// The wallet's showPasswordModal only supports input fields.
// We monkey-patch / override it to also support select dropdowns.
const _originalShowPasswordModal = typeof showPasswordModal === 'function' ? showPasswordModal : null;

if (_originalShowPasswordModal) {
    // Save original and wrap with enhanced version that supports select fields
    window._baseShowPasswordModal = _originalShowPasswordModal;
}

// Enhanced showPasswordModal that supports type='select' fields with options
window.showPasswordModal = function(options) {
    // Rewrite select fields to custom HTML, and patch post-render
    const enhancedFields = [];
    const selectFieldDefs = [];
    
    for (const field of (options.fields || [])) {
        if (field.type === 'select' && Array.isArray(field.options)) {
            // Convert to a text field placeholder, then patch after render
            enhancedFields.push({ ...field, type: 'text', placeholder: '(loading...)' });
            selectFieldDefs.push(field);
        } else {
            enhancedFields.push(field);
        }
    }
    
    const result = (window._baseShowPasswordModal || _originalShowPasswordModal)({
        ...options,
        fields: enhancedFields
    });
    
    // After modal renders, replace text inputs with selects
    if (selectFieldDefs.length > 0) {
        setTimeout(() => {
            for (const sf of selectFieldDefs) {
                const input = document.getElementById(sf.id);
                if (!input) continue;
            ...
            // Also set pre-filled values for text/number fields
            for (const f of (options.fields || [])) {
                if (f.value !== undefined && f.type !== 'select') {
...
// Dashboard integration is handled in wallet.js setupDashboardTabs()
console.log('MoltyID Identity module loaded');
