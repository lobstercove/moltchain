...
// Single instruction, single account, small data
{
    ...
    // compute_budget: Option<u64> = None (0x00)
    assertEqual(msg[off], 0x00, 'compute_budget = None');
    off += 1;
    // compute_unit_price: Option<u64> = None (0x00)
    assertEqual(msg[off], 0x00, 'compute_unit_price = None');
    off += 1;

    assertEqual(off, msg.length, 'No trailing bytes');
}
...
const genesisSrc = fs.readFileSync(__dirname + '/../genesis/src/lib.rs', 'utf8');
...
const genesisSource = fs.readFileSync(__dirname + '/../genesis/src/lib.rs', 'utf-8');
...
assert(
    genesisSource.includes('return false;') &&
    genesisSource.includes('contract returned error code'),
...
// P2.8: Genesis pool_configs has 7 entries (no REEF) (F2.9)
{
    assert(
        ...
        'P2.8: Genesis pool_configs array has 7 entries'
    );
...
assert(
    genesisSource.includes('(1 << 32) * sqrt(real_price)'),
...
assert(
    dexSource.includes('`ticker:${p.pairId}`') && dexSource.includes('d.lastPrice'),
...
const genesisSrcAll = fs.readFileSync(__dirname + '/../genesis/src/lib.rs', 'utf8');
...
assert(
    validatorSrcAll.includes('Active market: trades drive displayed prices'),
    'PB.3: Oracle feeder skips ana_lp_ write for active markets'
);

// PB.4: Inactive markets still get oracle indicative price
assert(
    validatorSrcAll.includes('Inactive market: write indicative price from oracle'),
...
assert(
    ...
    validatorSrcAll.includes('band_data.extend_from_slice(&slot.to_le_bytes())'),
...
assert(
    genesisSrcAll.includes('fn genesis_seed_analytics_prices'),
