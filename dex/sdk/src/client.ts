...
const DEFAULT_WS_ENDPOINT = 'ws://localhost:8900';
...
export class LichenDEX {
  ...
  /** Open a margin position (redirects to sendTransaction) */
  ...
  /** Close a margin position (redirects to sendTransaction) */
  ...
  /** Add margin to an existing position via signed sendTransaction.
   *  Use encodeAddMargin() from the margin module to build calldata. */
  async addMargin(positionId: number, amount: number): Promise<ApiResponse<MarginPosition>> {
    return this.post('/api/v1/margin/open', { positionId, amount, action: 'add_margin' });
  }
