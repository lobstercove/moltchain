...
async function displayTransaction(tx) {
    ...
    document.getElementById('baseFee').textContent = isFeeFree
        ...
        : formatMolt(BASE_FEE_SHELLS) + ' (' + formatShells(BASE_FEE_SHELLS) + ')';
    const zkComputeFee = ZK_COMPUTE_FEE[typeRaw.toLowerCase()] || 0;
    document.getElementById('feeNote').textContent = isFeeFree
        ? 'System reward/repay transactions are fee-free'
        : zkComputeFee > 0
            ? `Fee includes base fee (${formatShells(BASE_FEE_SHELLS)}) + shielded verification compute (${formatShells(zkComputeFee)}). Reputation discounts may reduce total.`
            : 'Fee split is applied to this transaction';
    const feeBurned = Math.floor(fee * FEE_SPLIT.burned);
    const feeProducer = Math.floor(fee * FEE_SPLIT.producer);
    const feeVoters = Math.floor(fee * FEE_SPLIT.voters);
    const feeValidatorPool = Math.floor(fee * FEE_SPLIT.treasury);
