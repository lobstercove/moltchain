// PredictionReef — Adversarial & Edge Case Tests
//
...
fn test_circuit_breaker_arms_on_large_price_move() {
    ...
    // A 5 mUSD buy on 10 mUSD pool is huge (50%) → triggers circuit breaker
...
fn test_small_trade_does_not_trigger_breaker() {
    ...
    // Small trade relative to 100 mUSD pool
...
fn test_buy_shares_caller_mismatch() {
    ...
    let real = [2u8; 32];
    let fake = [3u8; 32];
    moltchain_sdk::test_mock::set_caller(real);
...
fn test_sell_exactly_all_shares() {
    ...
    assert!(returned > 0, "Selling all shares should return mUSD");
...
fn test_sell_after_close_slot() {
    ...
    moltchain_sdk::test_mock::set_caller(t);
    buy_shares(t.as_ptr(), mid, 0, 1_000_000);
    moltchain_sdk::test_mock::set_slot(1000 + 100_001);
    moltchain_sdk::test_mock::set_caller(t);
    let (pos, _) = read_position(mid, &t, 0);
...
fn test_multi_outcome_buy_all_outcomes() {
    ...
    for outcome in 0..4u8 {
        moltchain_sdk::test_mock::set_slot(1000 + (outcome as u64) * 200); // advance slot each time
        moltchain_sdk::test_mock::set_caller(t);
        moltchain_sdk::test_mock::set_value(1_000_000);
...
fn test_multi_outcome_mint_complete_set() {
    ...
    let t = [2u8; 32];
    moltchain_sdk::test_mock::set_caller(t);
    moltchain_sdk::test_mock::set_value(5_000_000);
...
fn test_many_traders_sequential() {
    ...
    for i in 0..20u8 {
        let mut addr = [0u8; 32];
        addr[0] = i + 10;
        moltchain_sdk::test_mock::set_slot(1000 + (i as u64) * 200); // advance slot to avoid breaker
        moltchain_sdk::test_mock::set_caller(addr);
        let outcome = i % 2;
