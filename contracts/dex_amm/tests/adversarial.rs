...
fn setup_with_pool() -> ([u8; 32], u64) {
    ...
    assert_eq!(
        create_pool(admin.as_ptr(), ta.as_ptr(), tb.as_ptr(), 2, sqrt_price),
        0
    ); // FEE_TIER_30BPS
    (admin, 1)
}
...
fn test_create_pool_duplicate_pair() {
    ...
    assert_eq!(
        create_pool(admin.as_ptr(), ta.as_ptr(), tb.as_ptr(), 2, price),
        0
    );
    let same_order = create_pool(admin.as_ptr(), ta.as_ptr(), tb.as_ptr(), 2, price);
    assert_eq!(
        same_order, 6,
        "duplicate pair (same order) must be rejected"
    );

    let reversed_order = create_pool(admin.as_ptr(), tb.as_ptr(), ta.as_ptr(), 2, price);
    assert_eq!(
        reversed_order, 6,
        "duplicate pair (reversed order) must be rejected"
    );
}
...
fn test_create_pool_zero_sqrt_price() {
    ...
    let tb = [20u8; 32];
    assert_eq!(
        create_pool(admin.as_ptr(), ta.as_ptr(), tb.as_ptr(), 2, 0),
        4,
...
fn test_add_liquidity_out_of_range_ticks() {
    ...
    assert_eq!(
        add_liquidity(provider.as_ptr(), pool_id, 0, 887_280, 100_000, 100_000),
        3
    );
}
...
fn test_add_liquidity_below_minimum() {
    ...
    assert_eq!(
        result, 4,
        "tiny amounts should be rejected with MIN_AMOUNT check"
    );
}
...
fn test_remove_liquidity_more_than_owned() {
    ...
    assert_eq!(
        remove_liquidity(provider.as_ptr(), 1, u64::MAX),
        3,
...
fn test_swap_zero_liquidity_pool() {
    ...
    assert_eq!(
        result, 4,
        "swap on zero-liquidity pool should fail slippage check"
    );
}
...
fn test_swap_slippage_protection() {
    ...
    assert_eq!(
        result, 4,
        "slippage check should reject when min_out is too high"
    );
}
...
fn test_set_pool_protocol_fee_above_100() {
    ...
    assert_eq!(
        set_pool_protocol_fee(admin.as_ptr(), pool_id, 101),
        2,
...
fn test_swap_very_large_amount() {
    ...
    assert!(
        result == 0 || result == 4 || result == 6,
        "extreme swap should not panic, result={}",
        result
    );
}
...
fn test_add_liquidity_u64_max_amounts() {
    ...
    assert!(
        ...
        "extreme amounts should not panic, result={}",
        result
    );
}
...
fn test_quote_swap_nonexistent_pool() {
    let _admin = setup();
    assert_eq!(
        quote_swap(999, true, 1000),
        0,
        "quote on nonexistent pool should return 0"
    );
}
