...
fn is_ma_admin(caller: &[u8]) -> bool {
    storage_get(MA_ADMIN_KEY).map(|d| d.as_slice() == caller).unwrap_or(false)
}
...
pub extern "C" fn create_auction(
    ...
    let auction_duration = if duration > 0 { duration } else { AUCTION_DURATION };
...
#[no_mangle]
pub extern "C" fn update_collection_stats(
    nft_contract_ptr: *const u8,
    sale_price: u64,
) -> u32 {
...
#[no_mangle]
pub extern "C" fn get_collection_stats(
    nft_contract_ptr: *const u8,
    result_ptr: *mut u8,
) -> u32 {
    let mut nft_contract = [0u8; 32];
...
mod tests {
    ...
    fn create_test_auction(nft_contract: &[u8; 32], token_id: u64, seller: &[u8; 32], min_bid: u64, end_time: u64) {
    ...
    fn test_create_auction_nft_check_fails() {
        ...
        assert_eq!(create_auction(seller.as_ptr(), nft.as_ptr(), 1, 1000, pay.as_ptr(), 3600), 0);
    }
    ...
    fn test_accept_offer_ownership_fails() {
        ...
        assert_eq!(accept_offer(seller.as_ptr(), offerer.as_ptr(), nft.as_ptr(), 1), 0);
    }
    ...
    fn test_update_and_get_collection_stats() {
        ...
        assert_eq!(get_collection_stats(nft.as_ptr(), result_buf.as_mut_ptr()), 1);
    ...
    fn test_get_collection_stats_empty() {
        ...
        let nft = [3u8; 32];
        let mut result_buf = [0u8; 24];
        assert_eq!(get_collection_stats(nft.as_ptr(), result_buf.as_mut_ptr()), 0);
    }
