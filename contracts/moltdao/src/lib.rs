...
pub extern "C" fn create_proposal_typed(
    ...
    log_info(&alloc::format!(
        "   Proposal stake required: {} shells",
        min_threshold
    ));
...
#[no_mangle]
pub extern "C" fn set_moltyid_address(_caller_ptr: *const u8, moltyid_addr_ptr: *const u8) -> u32 {
    let real_caller = get_caller();
    let owner = storage_get(b"dao_owner").unwrap_or_default();
    ...
    let mut addr = [0u8; 32];
    unsafe {
        core::ptr::copy_nonoverlapping(moltyid_addr_ptr, addr.as_mut_ptr(), 32);
    }
