// mUSD Token — Adversarial & Hardening Tests
//
...
fn test_epoch_cap_single_large_mint() {
    ...
    // MINT_CAP_PER_EPOCH = 100_000_000_000 (100K mUSD in micro-units)
...
fn test_epoch_cap_reset_after_epoch() {
    ...
    moltchain_sdk::test_mock::set_slot(100 + 86_401);

    // Should be able to mint again in new epoch
    let result = mint(admin.as_ptr(), user.as_ptr(), 100_000_000_000);
...
fn test_epoch_cap_multiple_small_mints() {
    ...
    // Mint in chunks
    for _ in 0..10 {
        assert_eq!(mint(admin.as_ptr(), user.as_ptr(), 10_000_000_000), 0);
    }
...
fn test_circuit_breaker_blocks_minting_when_underbacked() {
    ...
    let result = mint(admin.as_ptr(), user.as_ptr(), 1);
    assert_eq!(result, 10, "circuit breaker should block minting when underbacked");
}
...
fn test_circuit_breaker_allows_when_fully_backed() {
    ...
    let proof = [42u8; 32];
    assert_eq!(attest_reserves(admin.as_ptr(), 200_000_000_000, proof.as_ptr()), 0);
...
fn test_attest_reserves_zero_blocks_everything() {
    ...
    assert!(result == 0 || result == 10,
        "attesting 0 reserves: result={} (0=breaker disabled, 10=breaker active)", result);
}
...
fn test_transfer_to_zero_address() {
    ...
    moltchain_sdk::test_mock::set_slot(100);
    assert_eq!(mint(admin.as_ptr(), user.as_ptr(), 1_000_000), 0);
    assert_eq!(transfer(user.as_ptr(), zero.as_ptr(), 100), 3,
...
fn test_transfer_zero_amount() {
    ...
    assert_eq!(transfer(user1.as_ptr(), user2.as_ptr(), 0), 4,
...
fn test_transfer_insufficient_balance() {
    ...
    assert_eq!(transfer(user1.as_ptr(), user2.as_ptr(), 101), 5,
...
fn test_transfer_preserves_total_supply() {
    ...
    assert_eq!(total_supply(), supply_before, "total supply should not change on transfer");
}
...
fn test_approve_and_transfer_from() {
    ...
    assert_eq!(transfer_from(spender.as_ptr(), owner.as_ptr(), recipient.as_ptr(), 3000), 0);
...
fn test_transfer_from_exceeds_allowance() {
    ...
    assert_eq!(transfer_from(spender.as_ptr(), owner.as_ptr(), recipient.as_ptr(), 101), 7,
...
fn test_transfer_from_exceeds_balance() {
    ...
    assert_eq!(transfer_from(spender.as_ptr(), owner.as_ptr(), recipient.as_ptr(), 200), 5,
...
fn test_approve_self() {
    let owner = setup();
    assert_eq!(approve(owner.as_ptr(), owner.as_ptr(), 1000), 6,
...
fn test_approve_zero_spender() {
    let admin = setup();
    let zero = [0u8; 32];
    assert_eq!(approve(admin.as_ptr(), zero.as_ptr(), 1000), 3,
...
fn test_burn_updates_accounting() {
    ...
    assert_eq!(total_minted(), 1_000_000, "total_minted should not change on burn");
...
fn test_mint_non_admin() {
    ...
    let user = addr(2);
    moltchain_sdk::test_mock::set_slot(100);
    assert_eq!(mint(attacker.as_ptr(), user.as_ptr(), 1_000_000), 2,
...
fn test_transfer_admin_to_zero() {
    let admin = setup();
    let zero = [0u8; 32];
    assert_eq!(transfer_admin(admin.as_ptr(), zero.as_ptr()), 3,
...
fn test_mint_while_paused() {
    ...
    assert_eq!(mint(admin.as_ptr(), user.as_ptr(), 1000), 1,
...
fn test_transfer_while_paused() {
    ...
    assert_eq!(emergency_pause(admin.as_ptr()), 0);
    assert_eq!(transfer(user1.as_ptr(), user2.as_ptr(), 100), 1,
...
fn test_multiple_attestations() {
    ...
    let proof = [42u8; 32];
    assert_eq!(attest_reserves(admin.as_ptr(), 1_000_000, proof.as_ptr()), 0);
    assert_eq!(get_attestation_count(), 1);

    moltchain_sdk::test_mock::set_slot(200);
    assert_eq!(attest_reserves(admin.as_ptr(), 2_000_000, proof.as_ptr()), 0);
...
fn test_attest_non_admin() {
    ...
    let rando = addr(99);
    let proof = [42u8; 32];
    assert_eq!(attest_reserves(rando.as_ptr(), 1_000_000, proof.as_ptr()), 2);
}
...
fn test_epoch_remaining() {
    ...
    assert_eq!(remaining_before, 100_000_000_000);

    assert_eq!(mint(admin.as_ptr(), user.as_ptr(), 30_000_000_000), 0);

    let remaining_after = get_epoch_remaining();
    assert_eq!(remaining_after, 70_000_000_000, "remaining should decrease after mint");
}
