...
| 2.14 | A12-01 | PENDING | Feb 20 | Genesis distribution alignment: genesis.rs and multisig.rs already agree — validator_rewards=150M (15%), builder_grants=250M (25%), community_treasury=400M (40%), founding_moltys=100M (10%), ecosystem_partnerships=50M (5%), reserve_pool=50M (5%). Total 1B. validator/src/main.rs REWARD_POOL_MOLT=150M also consistent. Finding description was inaccurate about which values were swapped. 1 new regression test: `a12_01_genesis_distribution_matches_multisig` verifies all 6 allocation amounts appear in both genesis.rs and multisig.rs and sum to 1B. 448 Rust + 52 JS tests, 0 regressions. |
