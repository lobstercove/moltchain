...
fn musd_addr() -> [u8; 32] { let mut a = [0u8; 32]; a[0] = 100; a }
fn wsol_addr() -> [u8; 32] { let mut a = [0u8; 32]; a[0] = 101; a }
fn weth_addr() -> [u8; 32] { let mut a = [0u8; 32]; a[0] = 102; a }
...
fn test_phase1_genesis_all_contracts_initialize() {
    ...
    assert_eq!(dex_governance::initialize(admin.as_ptr()), 0, "dex_governance init");
    assert_eq!(dex_margin::initialize(admin.as_ptr()), 0, "dex_margin init");
    assert_eq!(dex_rewards::initialize(admin.as_ptr()), 0, "dex_rewards init");
    assert_eq!(dex_analytics::initialize(admin.as_ptr()), 0, "dex_analytics init");

    // Double-init should fail
    assert_ne!(musd_token::initialize(admin.as_ptr()), 0, "mUSD double-init");
    assert_ne!(dex_core::initialize(admin.as_ptr()), 0, "dex_core double-init");
}
...
fn test_phase2_token_mint_and_transfer() {
    ...
    assert_eq!(musd_token::attest_reserves(admin.as_ptr(), 10_000_000_000, ZERO_PROOF.as_ptr()), 0);

    // Mint tokens
    assert_eq!(musd_token::mint(admin.as_ptr(), alice.as_ptr(), 1_000_000), 0, "mint mUSD to alice");
    assert_eq!(musd_token::mint(admin.as_ptr(), bob.as_ptr(), 500_000), 0, "mint mUSD to bob");
    assert_eq!(wsol_token::mint(admin.as_ptr(), alice.as_ptr(), 50_000), 0, "mint wSOL to alice");
    assert_eq!(weth_token::mint(admin.as_ptr(), bob.as_ptr(), 25_000), 0, "mint wETH to bob");
    ...
    assert_eq!(musd_token::transfer(alice.as_ptr(), bob.as_ptr(), 200_000), 0);
    assert_eq!(musd_token::balance_of(alice.as_ptr()), 800_000);
    assert_eq!(musd_token::balance_of(bob.as_ptr()), 700_000);
    assert_eq!(musd_token::total_supply(), 1_500_000, "transfer preserves supply");

    // Approve + transfer_from: Alice approves Carol to spend 100k mUSD
    let carol = addr(CAROL);
    assert_eq!(musd_token::approve(alice.as_ptr(), carol.as_ptr(), 100_000), 0);
    assert_eq!(musd_token::allowance(alice.as_ptr(), carol.as_ptr()), 100_000);
    assert_eq!(musd_token::transfer_from(carol.as_ptr(), alice.as_ptr(), bob.as_ptr(), 50_000), 0);
    assert_eq!(musd_token::balance_of(alice.as_ptr()), 750_000);
    assert_eq!(musd_token::balance_of(bob.as_ptr()), 750_000);
    assert_eq!(musd_token::allowance(alice.as_ptr(), carol.as_ptr()), 50_000);
}
...
fn test_phase2_token_burn() {
    ...
    assert_eq!(musd_token::initialize(admin.as_ptr()), 0);
    assert_eq!(musd_token::attest_reserves(admin.as_ptr(), 1_000_000_000, ZERO_PROOF.as_ptr()), 0);
...
fn test_phase3_dex_core_order_lifecycle() {
    ...
    let base = wsol_addr();
    let quote = musd_addr();
    assert_eq!(dex_core::create_pair(
        admin.as_ptr(), base.as_ptr(), quote.as_ptr(),
        1_000_000, 100, 1000
    ), 0);
    ...
    assert_eq!(dex_core::place_order(alice.as_ptr(), 1, 1, 0, price, 200_000, 0), 0);

    // Bob buys 200_000 wSOL at price 1.0 → should match
    assert_eq!(dex_core::place_order(bob.as_ptr(), 1, 0, 0, price, 200_000, 0), 0);

    // Verify matching happened
    assert!(dex_core::get_trade_count() > 0, "trades should have been executed");
    assert!(dex_core::get_fee_treasury() > 0, "fees should have been collected");

    // Order 1 was fully filled (same qty both sides), so cancel should return 3
    assert_eq!(dex_core::cancel_order(alice.as_ptr(), 1), 3, "filled order can't be cancelled");
}
...
fn test_phase3_dex_core_multi_pair() {
    ...
    assert_eq!(dex_core::create_pair(
        admin.as_ptr(), wsol_addr().as_ptr(), musd_addr().as_ptr(),
        1_000_000, 100, 1000
    ), 0);
    assert_eq!(dex_core::create_pair(
        admin.as_ptr(), weth_addr().as_ptr(), musd_addr().as_ptr(),
        1_000_000, 100, 1000
    ), 0);
    ...
    let alice = addr(ALICE);
    assert_eq!(dex_core::place_order(alice.as_ptr(), 1, 0, 0, 1_000_000_000, 1100, 0), 0);
    assert_eq!(dex_core::place_order(alice.as_ptr(), 2, 0, 0, 2_000_000_000, 1100, 0), 0);
}
...
fn test_phase4_amm_pool_lifecycle() {
    ...
    assert_eq!(dex_amm::create_pool(
        admin.as_ptr(), token_a.as_ptr(), token_b.as_ptr(), 1, 1u64 << 32
    ), 0);
    assert_eq!(dex_amm::get_pool_count(), 1);

    // Alice adds liquidity
    assert_eq!(dex_amm::add_liquidity(
        alice.as_ptr(), 1, -120, 120, 500_000, 500_000
    ), 0);
    assert_eq!(dex_amm::get_position_count(), 1);

    // TVL should be nonzero
    assert!(dex_amm::get_tvl(1) > 0, "TVL should reflect deposited liquidity");

    // Bob swaps: exact_in 10_000 through the pool
    let result = dex_amm::swap_exact_in(
        bob.as_ptr(), 1, true, 10_000, 0, 1000
    );
    ...
    assert!(collect_result == 0 || collect_result == 3, "fee collection: {}", collect_result);
...
fn test_phase5_margin_position_lifecycle() {
    ...
    assert_eq!(dex_margin::open_position(
        alice.as_ptr(), 1, 0, 1000, 2, 200
    ), 0);
    ...
    assert!(new_ratio > ratio, "ratio should increase after adding margin");
    ...
    assert_eq!(dex_margin::open_position(
        bob.as_ptr(), 1, 1, 10_000, 5, 400
    ), 0);
    // Jack price up 10x to make short liquidatable
    dex_margin::set_mark_price(admin.as_ptr(), 1, 10_000_000_000);
    let carol = addr(CAROL);
    assert_eq!(dex_margin::liquidate(carol.as_ptr(), 2), 0, "short should be liquidatable");
    assert!(dex_margin::get_insurance_fund() > 0, "insurance fund should have balance");
}
...
fn test_phase6_analytics_trade_recording() {
    ...
    // Record some trades
    assert_eq!(dex_analytics::record_trade(1, 1_000_000_000, 50_000, alice.as_ptr()), 0);
    test_mock::set_slot(101);
    assert_eq!(dex_analytics::record_trade(1, 1_010_000_000, 30_000, alice.as_ptr()), 0);
    test_mock::set_slot(102);
    assert_eq!(dex_analytics::record_trade(1, 990_000_000, 20_000, alice.as_ptr()), 0);
...
fn test_phase7_rewards_trade_and_claim() {
    ...
    assert!(dex_rewards::get_total_distributed() > 0, "rewards distributed");

    // Register referral: Bob referred by Alice
    assert_eq!(dex_rewards::register_referral(bob.as_ptr(), alice.as_ptr()), 0);
...
fn test_phase8_governance_proposal_lifecycle() {
    ...
    let base = wsol_addr();
    let quote = musd_addr();
    assert_eq!(dex_governance::propose_new_pair(
        alice.as_ptr(), base.as_ptr(), quote.as_ptr()
    ), 0);
    ...
    // Propose fee change
    assert_eq!(dex_governance::propose_fee_change(admin.as_ptr(), 1, 0, 10), 0);
...
fn test_phase9_router_route_and_swap() {
    ...
    assert_eq!(dex_router::set_addresses(
        admin.as_ptr(), core_addr.as_ptr(), amm_addr.as_ptr(), legacy_addr.as_ptr()
    ), 0);

    // Register route: wSOL→mUSD via CLOB pair 1
    assert_eq!(dex_router::register_route(
        admin.as_ptr(), wsol_addr().as_ptr(), musd_addr().as_ptr(),
        0, 1, 0, 100
    ), 0);
    ...
    let result = dex_router::swap(
        alice.as_ptr(), wsol_addr().as_ptr(), musd_addr().as_ptr(),
        10_000, 0, 1000
    );
...
fn test_phase10_full_pipeline() {
    ...
    // === MINT TOKENS ===
    assert_eq!(musd_token::attest_reserves(admin.as_ptr(), 100_000_000_000, ZERO_PROOF.as_ptr()), 0);
    assert_eq!(musd_token::mint(admin.as_ptr(), alice.as_ptr(), 10_000_000), 0);
    assert_eq!(musd_token::mint(admin.as_ptr(), bob.as_ptr(), 5_000_000), 0);
    assert_eq!(wsol_token::mint(admin.as_ptr(), alice.as_ptr(), 1_000_000), 0);
    ...
    // === PLACE & MATCH ORDERS ===
    let price = 1_000_000_000u64;
    assert_eq!(dex_core::place_order(alice.as_ptr(), 1, 1, 0, price, 200_000, 0), 0); // sell
    assert_eq!(dex_core::place_order(bob.as_ptr(), 1, 0, 0, price, 200_000, 0), 0);   // buy
    let trades = dex_core::get_trade_count();
    assert!(trades > 0, "orders should have matched");

    // === RECORD ANALYTICS ===
    assert_eq!(dex_analytics::record_trade(1, price, 200_000, alice.as_ptr()), 0);
    ...
    // === CREATE AMM POOL ===
    assert_eq!(dex_amm::create_pool(
        admin.as_ptr(), wsol_addr().as_ptr(), musd_addr().as_ptr(), 1, 1u64 << 32
    ), 0);
    assert_eq!(dex_amm::add_liquidity(alice.as_ptr(), 1, -120, 120, 100_000, 100_000), 0);

    // === OPEN MARGIN POSITION ===
    dex_margin::set_mark_price(admin.as_ptr(), 1, price);
    assert_eq!(dex_margin::open_position(alice.as_ptr(), 1, 0, 1000, 2, 200), 0);

    // === GOVERNANCE PROPOSAL ===
    assert_eq!(dex_governance::propose_new_pair(
        alice.as_ptr(), weth_addr().as_ptr(), musd_addr().as_ptr()
    ), 0);
    assert_eq!(dex_governance::vote(alice.as_ptr(), 1, true), 0);
    assert_eq!(dex_governance::vote(bob.as_ptr(), 1, true), 0);
...
fn test_phase11_emergency_pause_all() {
    ...
    let alice = addr(ALICE);
    assert_eq!(musd_token::attest_reserves(admin.as_ptr(), 1_000_000, ZERO_PROOF.as_ptr()), 0);
    assert_ne!(musd_token::mint(admin.as_ptr(), alice.as_ptr(), 1000), 0, "mint should fail while paused");
    ...
    assert_eq!(musd_token::mint(admin.as_ptr(), alice.as_ptr(), 1000), 0, "mint after unpause");
}
...
fn test_phase12_access_control_all_contracts() {
    ...
    assert_ne!(musd_token::mint(rando.as_ptr(), rando.as_ptr(), 1000), 0, "token mint non-admin");
    assert_ne!(dex_core::create_pair(
        rando.as_ptr(), wsol_addr().as_ptr(), musd_addr().as_ptr(), 1, 1, 1
    ), 0, "create_pair non-admin");
    assert_ne!(dex_amm::create_pool(
        rando.as_ptr(), wsol_addr().as_ptr(), musd_addr().as_ptr(), 1, 1 << 32
    ), 0, "create_pool non-admin");
    assert_ne!(dex_margin::set_mark_price(rando.as_ptr(), 1, 1000), 0, "set_mark_price non-admin");
    assert_ne!(dex_core::emergency_pause(rando.as_ptr()), 0, "emergency_pause non-admin");
    assert_ne!(dex_router::emergency_pause(rando.as_ptr()), 0, "router pause non-admin");
    assert_ne!(dex_governance::emergency_delist(rando.as_ptr(), 1), 0, "delist non-admin");
}
