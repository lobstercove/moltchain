...
> **Decision**: Every external asset deposited into MoltChain becomes a **wrapped receipt token** backed 1:1 by reserves in the treasury custody boundary. Treasury withdrawals now have live threshold paths on Solana and EVM, while deposit sweeps are still performed with locally derived deposit keys. mUSD is the unified quote asset; wSOL and wETH are the base trading assets.
