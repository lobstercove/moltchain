...
**Status:** Historical planning baseline, superseded where it conflicts with the aligned v0.4.x chain  
**Branch:** `main`  
**Commit baseline:** `8710e12`

> Historical plan note: this milestone document preserves February 2026 assumptions.
> The live chain now uses a 500M LICN genesis supply, protocol inflation settled at epoch boundaries,
...
- **Tokenomics alignment** → fix genesis distribution mismatch between whitepaper/multisig.rs and genesis.rs/website, readjust all LICN-denominated parameters for launch price
...
- **LICN auto-listing** → create LICN/lUSD trading pair at genesis
...
|----------|--------|-----------|
| DEX quote currency | **lUSD** for all trading pairs | Stablecoin-denominated trading, consistent pricing |
| Launchpad currency | **LICN** for token purchases | Drives LICN demand, increases utility and volume |
| User onramp | Buy LICN with USDT/USDC/SOL/ETH via bridge + wrapped tokens | Already planned in core code and bridge contract |
| Graduation currency | **LICN** (1M LICN market cap = $100K) | Consistent with launchpad denomination |
...
```
External Chain                    Lichen
─────────────                    ─────────
USDT/USDC  ──bridge──►  lUSD (wrapped stablecoin, 6 decimals)
SOL        ──bridge──►  WSOL (wrapped SOL, 9 decimals)
ETH        ──bridge──►  WETH (wrapped ETH, 9 decimals)

lUSD ──DEX swap──► LICN     (primary pair: LICN/lUSD)
WSOL ──DEX swap──► LICN     (via WSOL/lUSD → lUSD/LICN route)
WETH ──DEX swap──► LICN     (via WETH/lUSD → lUSD/LICN route)
```

### Trading Pairs (All quoted in lUSD)

| Pair | Template | Created At |
|------|----------|-----------|
| LICN/lUSD | Primary | Genesis (auto-listed) |
| WSOL/lUSD | Wrapped | Genesis (auto-listed) |
| WETH/lUSD | Wrapped | Genesis (auto-listed) |
| TOKEN/lUSD | User-deployed | Via governance or SporePump graduation |

### Launchpad Flow (SporePump — denominated in LICN)

```
1. Creator calls create_token() → pays 10 LICN creation fee
2. Users buy tokens with LICN on bonding curve
3. Market cap reaches 1,000,000 LICN → token graduates
4. On graduation:
   a. Accumulated LICN from bonding curve is used as liquidity
   b. Token pair TOKEN/lUSD created on dex_core
   c. Pool TOKEN/lUSD created on dex_amm
   d. Initial liquidity seeded (LICN converted to lUSD equivalent)
5. After graduation: buy/sell on bonding curve blocked → trade on DEX only
```

### Fee Burn Mechanism

- 40% of all transaction fees are burned (LICN is deflationary)
- LICN supply is governed at the protocol layer, with epoch-boundary minting plus fee burn
- Fee burn reduces circulating supply over time even while epoch minting may increase total supply
...
**Historical problem:** the February 2026 plan assumed stale genesis distribution values. The aligned chain now uses the canonical 500M genesis distribution below.
...
- `WHITEPAPER.md` — fix stale deploy fee (says 0.0001 LICN, should be 25 LICN at $0.10)

**Canonical genesis distribution (500M LICN total):**

| Allocation | LICN | % | Purpose |
|-----------|------|---|--------|
| Community Treasury | 125,000,000 | 25% | Ecosystem growth, governed by DAO |
| Builder Grants | 175,000,000 | 35% | DEX rewards, launchpad incentives |
| Validator Rewards | 50,000,000 | 10% | Legacy reserve wallet retained at genesis |
| Founding Team | 50,000,000 | 10% | 6-mo cliff + 18-mo vest |
| Ecosystem Partnerships | 50,000,000 | 10% | Strategic partners |
| Reserve Pool | 50,000,000 | 10% | Emergency buffer |

### Task 0.2 — Rename ANNUAL_INFLATION_BPS

**Historical problem:** the pre-alignment plan assumed inflation should be renamed away because LICN was treated as fixed-supply.

**Aligned live behavior:** the protocol is inflationary with fee-burn counter-pressure. Inflation accrues during the epoch and settles at epoch boundaries; it is not drawn from a fixed validator-rewards pool.

### Task 0.3 — Launch Price Readjustment ($0.10/LICN — LOCKED)

> **Price: $0.10 per LICN — LOCKED IN**
> **FDV at 500M genesis:** $50,000,000 (500M × $0.10)
> **Launch: tradeable from block 0 on DEX, no private round, fair launch**

#### How the original values were set

**The entire system was designed at $1.00/LICN.** Evidence:
...
- Block rewards: 0.1 LICN = $0.01/block at $0.10, designed to emit ~1.42% of supply/year at 100% activity
- Heartbeat: 0.05 LICN = 50% of tx reward — design comment says "50% of transaction reward"
...
**At $0.10, every value is 10× too cheap in USD.** We must multiply LICN amounts by 10 to hit the same USD targets.

#### Category A — Core Chain Fees (MUST 10×)

These target specific USD prices documented on the website and whitepaper.

| # | Parameter | File:Line | Current Spores | Current LICN | USD at $1 | USD at $0.10 | **New LICN** | **New Spores** | USD Result |
|---|-----------|-----------|---------------|-------------|-----------|-------------|------------|--------------|------------|
...
| 6 | `base_fee_spores` | genesis.rs:~120 | 10,000 | — | — | — | — | **1,000,000** | (match #1) |
| 7 | `rent_rate_spores_per_kb_month` | genesis.rs:~125 | 1,000 | 0.000001 | $0.000001 | $0.0000001 | **0.00001** | **10,000** | $0.000001 |

#### Category B — Block Rewards (DECISION: 5× recommended)

Block rewards come from the **validator rewards pool (100M LICN)**, not minted. At $1 the original design intent was:
- 0.18 LICN/block = $0.18 per transaction block
- 0.027 LICN/block = $0.027 per heartbeat (15% of tx reward)

> **✅ Applied:** 5× upgrade now live — 0.1 LICN/tx, 0.05 LICN/heartbeat

**At $0.10, keeping the same LICN amounts gives validators only $0.018/block — 10× less than designed.**

Pool depletion analysis at various multipliers (assuming 3 validators, 50% tx activity):

| Variant | TX Reward | Heartbeat | USD/block | Annual Draw (LICN) | Pool Life (100M) |
|---------|-----------|-----------|-----------|-------------------|------------------|
| 1× (no change) | 0.18 LICN | 0.027 LICN | $0.018 | ~8.1M | **~18.4 years** |
| 2× | 0.36 LICN | 0.054 LICN | $0.036 | ~16.3M | **~9.2 years** |
| **5× (recommended)** | **0.1 LICN** | **0.05 LICN** | **$0.01** | **~5.9M** | **~16.9 years** |
| 10× (full USD match) | 1.8 LICN | 0.27 LICN | $0.18 | ~81.3M | **~1.8 years** |
...
| # | Parameter | File:Line | Current Spores | Current LICN | **New LICN** | **New Spores** | USD at $0.10 |
|---|-----------|-----------|---------------|-------------|------------|--------------|-------------|
...
Original design: $10K min / $100K max to run a validator. At $0.10 those targets require 10× the LICN.

| # | Parameter | File:Line | Current LICN | USD at $0.10 | **New LICN** | USD at $0.10 |
|---|-----------|-----------|-------------|-------------|------------|-------------|
...
|---|-----------|------|---------|-------------|-------------|-------------|----------|
| 13 | `MAX_ORDER_SIZE` | dex_core:48 | 1,000 LICN | $100 | **10,000,000 LICN** | **$1,000,000** | Real DEX needs $1M max orders |
| 14 | `CREATION_FEE` | sporepump:42 | 0.1 LICN | $0.01 | **10 LICN** | **$1.00** | Anti-spam token creation |
| 15 | `GRADUATION_MARKET_CAP` | sporepump:48 | 100,000 LICN | $10,000 | **1,000,000 LICN** | **$100,000** | Original $100K target |
| 16 | `DEFAULT_MAX_BUY_AMOUNT` | sporepump:72 | 10,000 LICN | $1,000 | **100,000 LICN** | **$10,000** | Anti-whale cap |
| 17 | `MIN_LISTING_LIQUIDITY` | dex_governance:31 | 10 LICN | $1 | **100,000 LICN** | **$10,000** | Fix 1000× bug + scale |
| 18 | `REWARD_POOL_PER_MONTH` | dex_rewards:26 | 1,000,000 LICN | $100K/mo | **100,000 LICN** | **$10K/mo** | Draws from 350M builder grants, lasts 290+ years |
| 19 | `TIER_BRONZE_MAX` | dex_rewards:30 | 10,000 LICN vol | $1K vol | **100,000 LICN** | **$10K vol** | Scale volume tiers |
| 20 | `TIER_SILVER_MAX` | dex_rewards:31 | 100,000 LICN vol | $10K vol | **1,000,000 LICN** | **$100K vol** | Scale volume tiers |
| 21 | `TIER_GOLD_MAX` | dex_rewards:32 | 1,000,000 LICN vol | $100K vol | **10,000,000 LICN** | **$1M vol** | Scale volume tiers |

#### Category E — DAO & Governance (10×)

| # | Parameter | File | Current | USD at $0.10 | **New Value** | USD at $0.10 | Rationale |
|---|-----------|------|---------|-------------|-------------|-------------|----------|
| 22 | `PROPOSAL_STAKE` | lichendao:43 | 1,000 LICN | $100 | **10,000 LICN** | **$1,000** | Serious governance barrier |

#### Category F — Infrastructure (10×)

| # | Parameter | File | Current | **New Value** | Rationale |
|---|-----------|------|---------|-------------|----------|
| 23 | `MIN_STAKE_PER_GB` | moss_storage:42 | 0.001 LICN | **0.01 LICN** | 10× for storage provider stake |
| 24 | `REWARD_PER_SLOT_PER_BYTE` | moss_storage:37 | 1 spore | **10 spores** | 10× for storage rewards |
...
| 27 | `REWARD_POOL_LICN` | main.rs:43 | 150,000,000 | **100,000,000** | Updated — genesis allocation |

#### Category H — Faucet (testnet only)

| # | Parameter | File | Current | **New Value** | Rationale |
|---|-----------|------|---------|-------------|----------|
| 28 | `max_per_request` | faucet main.rs | 10 LICN | **100 LICN** | 10× so testers get $10 worth |
| 29 | `daily_limit_per_ip` | faucet main.rs | 10 LICN | **100 LICN** | Match above |
| 30 | `LICN_PER_REQUEST` | faucet.js | 10 | **100** | Match above |
...
| Lending LTV/rates | thalllend | % and bps | Interest rates |
...
| Reputation scores | lichenid | points | Not LICN |
| SporePump platform fee | sporepump | 1% | Percentage-based |
| Vault deposit/withdrawal fees | sporevault | 10/30 bps | Percentage-based |
| Referral rates | dex_rewards | bps | Percentage-based |
| Voting thresholds | dex_governance, lichendao | % | Percentage-based |
| Bonding curve slope | sporepump | ratio | Price discovery |
| Flash loan fee | thalllend, lichenswap | 9 bps | Percentage-based |
| Marketplace fee | lichenmarket, lichenauction | 250 bps | Percentage-based |
| All timeouts/cooldowns | various | slots | Time-based |
| MIN_ORDER_VALUE | dex_core | 1,000 spores | Already negligible |

#### Reward Pool Sustainability Summary

| Source | Pool | Monthly Draw | Depletes In |
|--------|------|-------------|-------------|
| Block rewards (5× at $0.10) | Validator Rewards (100M) | ~3.4M LICN | **~2.4 years** (50% activity) |
| DEX trading rewards | Builder Grants (350M) | 100K LICN | **~290 years** |
...
- **3 SporePump values** in `sporepump`
- **1 DAO value** in `lichendao`
- **2 infrastructure values** in `moss_storage`
...
**Problem:** wsol_token (86 B), weth_token (86 B), and lusd_token (86 B) have real source code (585–881 lines each with full logic) but compile to empty 86-byte WASM because functions use `pub fn` instead of `#[no_mangle] pub extern "C" fn`.

**Files to modify:**
- `contracts/wsol_token/src/lib.rs` (585 lines)
- `contracts/weth_token/src/lib.rs` (585 lines)
- `contracts/lusd_token/src/lib.rs` (881 lines)

**Fix:** Add `#[no_mangle] pub extern "C" fn` annotation to every public function. The reference implementation is `lichencoin/src/lib.rs` which uses this pattern correctly and compiles to 5,392 bytes.
...
lusd_token (17+ functions):
- Same as above plus proof-of-reserves attestation count tracking

**Validation:** After recompile, each WASM must be >1 KB (expected ~5–8 KB based on lichencoin reference).
...
1. **Tokens first** (no dependencies):
   - `lichencoin.initialize(deployer_pubkey)` — sets admin, creates initial supply
   - `lusd_token.initialize(deployer_pubkey)` — sets admin, zero initial supply
...
2. **Identity** (used by many contracts):
   - `lichenid.initialize(deployer_pubkey)` — sets admin for identity system
...
3. **DEX core** (depends on tokens):
   ...
   - `lichenswap.initialize(deployer_pubkey)` — sets admin for legacy AMM
   - `dex_router.initialize(deployer_pubkey)` — sets admin + addresses of dex_core, dex_amm, lichenswap
...
4. **DeFi protocols** (may reference DEX/tokens):
   - `thalllend.initialize(deployer_pubkey)` — lending protocol
   - `lichenbridge.initialize(deployer_pubkey)` — cross-chain bridge
   - `lichenoracle.initialize(deployer_pubkey)` — oracle feeds
   - `lichenswap.initialize(deployer_pubkey)` — legacy AMM (if not already done)

5. **Infrastructure** (may reference identity):
   - `lichendao.initialize(deployer_pubkey)` — DAO governance
   - `lichenmarket.initialize(deployer_pubkey)` — NFT marketplace
   - `lichenpunks.initialize(deployer_pubkey)` — NFT collection
   - `lichenauction.initialize(deployer_pubkey)` — auction house
   - `sporepump.initialize(deployer_pubkey)` — token launchpad
   - `sporevault.initialize(deployer_pubkey)` — yield aggregator
   - `sporepay.initialize(deployer_pubkey)` — streaming payments
   - `bountyboard.initialize(deployer_pubkey)` — bounty system
   - `compute_market.initialize(deployer_pubkey)` — compute marketplace
   - `moss_storage.initialize(deployer_pubkey)` — decentralized storage
...
### Task 1.3 — LICN Auto-Listing at Genesis

**File to modify:** `validator/src/main.rs` (after initialization phase)

**After all contracts are initialized, execute:**

1. `dex_core.create_pair("LICN", "LUSD")` — creates the primary CLOB trading pair
2. `dex_core.create_pair("WSOL", "LUSD")` — creates WSOL/lUSD pair
3. `dex_core.create_pair("WETH", "LUSD")` — creates WETH/lUSD pair
4. `dex_amm.create_pool("LICN", "LUSD", 30)` — creates 30bps fee tier AMM pool
5. `dex_amm.create_pool("WSOL", "LUSD", 30)` — creates WSOL/lUSD AMM pool
6. `dex_amm.create_pool("WETH", "LUSD", 30)` — creates WETH/lUSD AMM pool
...
### Task 1.4 — lUSD as DEX Quote Currency

**Files to modify:**
- `contracts/dex_core/src/lib.rs` — ensure `create_pair` validates quote token or add a preferred quote constant
- `contracts/dex_governance/src/lib.rs` — listing proposals should default to lUSD quote
- `contracts/dex_analytics/src/lib.rs` — 24h stats denominated in lUSD

**SporePump stays in LICN** (no change needed — already uses LICN).
...
- The Account struct already supports `lock()/unlock()` with invariant enforcement (`spores == spendable + staked + locked`)
...
- Transfers LICN from insurance fund to recipient via cross-contract call
- Emits event for transparency
- Future: governance proposal required (via dex_governance or lichendao)
...
- `execute_legacy_swap()` → calls `lichenswap.swap_a_for_b()` or `swap_b_for_a()`
...
### Task 2.6 — dex_rewards: Actual LICN Token Transfer on Claim

**File:** `contracts/dex_rewards/src/lib.rs` (609 lines)

**Problem:** `claim_trading_rewards()` and `claim_lp_rewards()` update internal bookkeeping but never actually transfer LICN tokens to the trader.

**Fix:**
- On `claim_trading_rewards()`: cross-contract call to `lichencoin.transfer(rewards_pool, trader, amount)`
- On `claim_lp_rewards()`: same pattern
- Need a rewards pool address (treasury or dedicated rewards wallet) that holds LICN for distribution
- Add validation: claim fails if rewards pool has insufficient balance

### Task 2.7 — SporePump: Real DEX Migration on Graduation

**File:** `contracts/sporepump/src/lib.rs` (1,109 lines)

**Problem:** When market cap hits 1,000,000 LICN, the contract just sets a `graduated` flag and blocks further bonding curve trades. No actual migration to DEX occurs.

**Fix — on graduation, execute:**
1. Cross-contract call: `dex_governance.propose_new_pair(token_symbol, "LUSD")` (auto-approved for graduated tokens)
   OR directly: `dex_core.create_pair(token_symbol, "LUSD")` if admin-privileged
2. Cross-contract call: `dex_amm.create_pool(token_symbol, "LUSD", 30)` (30bps fee tier)
3. Seed initial liquidity:
   - The bonding curve has accumulated LICN from buyers
   - Convert accumulated LICN to lUSD equivalent (via oracle price or dex_core last price)
   - Deposit as initial liquidity in the AMM pool
4. Emit graduation event with pair address, pool address, initial liquidity

**Graduation stays in LICN** — threshold is 1,000,000 LICN market cap ($100K at $0.10). The conversion to lUSD pair happens only when creating the DEX listing.

---

## 6. Phase 3 — Security & Cross-Contract Wiring

### Task 3.1 — lichenoracle: Secure Hash Function

**File:** `contracts/lichenoracle/src/lib.rs` (857 lines)
...
**Fix:** Cross-contract call to `lichenid.get_reputation(voter_address)` to fetch verified on-chain reputation. Reject caller-provided values.
...
**Fix:** Cross-contract call to `lichencoin.transfer(bounty_creator, winner, bounty_amount)` or for lUSD bounties: `lusd_token.transfer(...)`.

### Task 3.4 — sporevault: Real Yield Source Integration

**File:** `contracts/sporevault/src/lib.rs` (1,007 lines)
...
- Lending strategy → `thalllend.deposit()` / `thalllend.withdraw()`
- LP strategy → `lichenswap.add_liquidity()` / `lichenswap.remove_liquidity()`
...
3. Verify via RPC:
   ...
   - `getContractInfo("LICN")` shows `mintable: false, burnable: true`
   - `getContractInfo("LUSD")` shows storage entries > 0 (initialized)
   ...
   - Check dex_core has 3 pairs (LICN/lUSD, WSOL/lUSD, WETH/lUSD)
...
- Rename ANNUAL_INFLATION_BPS → ANNUAL_REWARD_RATE_BPS (LICN is not inflationary)
...
- Auto-list LICN/lUSD, WSOL/lUSD, WETH/lUSD pairs at genesis
...
- dex_rewards: actual LICN transfer on reward claims (source: builder_grants)
- sporepump: real DEX migration on graduation (create pair + pool)
- lichenoracle: replace insecure hash with cryptographic hash
- dex_governance: on-chain reputation verification via lichenid
- bountyboard: wire actual token transfer on work approval
- sporevault: connect to real yield sources (thalllend, lichenswap)
...
|---|----------|-------------|-----------|--------|---------------|
| 1 | lichencoin | 320 | 5.4 KB | OK | Yes — sets admin, initial supply |
| 2 | lusd_token | 881 | **86 B ⚠️** | BROKEN WASM | Yes — sets admin |
...
| 12 | sporepump | 1,109 | 17 KB | OK | Yes — sets admin, platform fees |
| 13 | lichenswap | 1,175 | 5.6 KB | OK | Yes — sets admin |
| 14 | lichenbridge | 1,921 | 17 KB | OK | Yes — sets admin, validator set |
| 15 | lichenoracle | 857 | 17 KB | OK | Yes — sets admin |
| 16 | lichenauction | 1,223 | 36 KB | OK | Yes — sets admin, marketplace fee |
| 17 | lichendao | 1,010 | 19 KB | OK | Yes — sets admin (initialize_dao) |
| 18 | lichenmarket | 742 | 8.7 KB | OK | Yes — sets admin, marketplace fee |
| 19 | lichenpunks | 500 | 9.2 KB | OK | Yes — sets admin |
| 20 | lichenid | 3,126 | 44 KB | OK | Yes — sets admin |
| 21 | thalllend | 1,203 | 12 KB | OK | Yes — sets admin, rates |
| 22 | moss_storage | 1,213 | 15 KB | OK | Yes — sets admin |
| 23 | bountyboard | 715 | 16 KB | OK | Yes — sets admin |
| 24 | compute_market | 1,687 | 17 KB | OK | Yes — sets admin |
| 25 | sporevault | 1,007 | 18 KB | OK | Yes — sets admin, fees |
| 26 | sporepay | 1,192 | 18 KB | OK | Yes — sets admin |

**Total source:** ~27,500 lines of Rust across 26 contracts  
**Broken WASMs:** 3 (wsol_token, weth_token, lusd_token — export annotation issue)  
...
| **CRITICAL** | wsol/weth/lusd_token | Missing `#[no_mangle] pub extern "C"` → 86-byte empty WASM | 1.1 |
| **CRITICAL** | All 26 contracts | Never initialized at genesis — empty storage, no admin | 1.2 |
| **HIGH** | consensus.rs | `ANNUAL_INFLATION_BPS` implies minting — LICN is NOT inflationary | 0.2 |
| **HIGH** | processor.rs | Deploy/upgrade fees need readjustment for $0.10/LICN ($2.50/$1.00 target) | 0.3 |
| **HIGH** | dex_core/dex_rewards/sporepump/dex_governance | Multiple parameter values misaligned for $0.10/LICN | 0.3 |
| **HIGH** | dex_router | Swap execution is simulated — no real cross-contract calls | 2.5 |
| **HIGH** | dex_rewards | Reward claims are bookkeeping only — no LICN transfer, no source wallet defined | 2.6 |
| **HIGH** | sporepump | Graduation just sets a flag — no DEX migration | 2.7 |
| **HIGH** | lichenoracle | `simple_hash` is not cryptographic — VRF is vulnerable | 3.1 |
...
| **MEDIUM** | dex_governance | `MIN_LISTING_LIQUIDITY` = 10 LICN, comment says 10,000 LICN (off by 1000×) | 0.3 |
| **MEDIUM** | bountyboard | `approve_work` doesn't transfer tokens | 3.3 |
| **MEDIUM** | sporevault | Yield generation is simulated with hardcoded APY | 3.4 |
| **LOW** | WHITEPAPER.md | Deploy fee listed as 0.0001 LICN (stale — should be 2.5 LICN) | 0.1 |
...
PHASE 3 — Create Trading Pairs (new)
  1. Call dex_core.create_pair(LICN_addr, LUSD_addr)
  2. Call dex_core.create_pair(WSOL_addr, LUSD_addr)
  3. Call dex_core.create_pair(WETH_addr, LUSD_addr)
  4. Call dex_amm.create_pool(LICN_addr, LUSD_addr, 30)
  5. Call dex_amm.create_pool(WSOL_addr, LUSD_addr, 30)
  6. Call dex_amm.create_pool(WETH_addr, LUSD_addr, 30)
```

**Dependency graph:**

```
Layer 0 (no deps):    lichencoin, lusd_token, wsol_token, weth_token
Layer 1 (tokens):     lichenid
Layer 2 (identity):   dex_core, dex_amm, lichenswap
Layer 3 (DEX core):   dex_router (needs dex_core + dex_amm + lichenswap addresses)
                      dex_margin, dex_rewards, dex_governance, dex_analytics
Layer 4 (protocols):  thalllend, lichenbridge, lichenoracle, lichendao
Layer 5 (apps):       lichenmarket, lichenpunks, lichenauction, sporepump,
                      sporevault, sporepay, bountyboard, compute_market, moss_storage
```
...
**How it works:**
- Trader requests 25x leverage on a 1000 lUSD position
- Notional value: 25,000 lUSD
- Initial margin required: 4% = 1000 lUSD (deposited and locked)
- Maintenance margin: 2% = 500 lUSD
- If position value drops such that remaining margin < 500 lUSD → liquidatable
- Liquidation penalty: 7% of notional = 1,750 lUSD (split: 875 to liquidator, 875 to insurance)
...
| lichenoracle mainnet hardening | Needs real cryptographic VRF (beyond hash fix) |
...
| Question | Answer |
|----------|--------|
| What currency for launchpad? | **LICN** — buy tokens with LICN on bonding curve |
| What currency for DEX trading? | **lUSD** — all trading pairs are TOKEN/lUSD |
| How do users get LICN? | Bridge USDT/USDC → lUSD, then swap lUSD → LICN on DEX |
| How do users get lUSD? | Bridge USDT/USDC from external chains via lichenbridge |
| Is LICN mintable? | **No** — fixed 1B supply, deflationary via 40% fee burn |
| Is LICN burnable? | **Yes** — fees burn mechanism |
| What happens on graduation? | Token gets DEX pair (TOKEN/lUSD), AMM pool created, initial liquidity seeded |
...
| What is fee mining? | Traders earn LICN proportional to fees paid (1:1 base × tier multiplier) |
...
```
 ...
 3. Task 0.3  — Readjust all LICN parameters for $0.10/LICN (LOCKED)

 PHASE 1 — Critical Fixes (Foundation)
 4. Task 1.1  — Fix 3 wrapped token exports + recompile WASMs
 5. Task 1.4  — lUSD as DEX quote currency enforcement
...
11. Task 2.6  — dex_rewards: actual LICN transfer (source: builder_grants)
12. Task 2.7  — sporepump: real DEX migration on graduation

 PHASE 3 — Security & Cross-Contract Wiring
13. Task 3.1  — lichenoracle: secure hash
14. Task 3.2  — dex_governance: on-chain reputation
15. Task 3.3  — bountyboard: token transfer
16. Task 3.4  — sporevault: real yield sources

 PHASE 1 (continued) — Genesis wiring (after all contracts enhanced)
17. Task 1.2  — Genesis initialization (all 26 contracts)
18. Task 1.3  — LICN auto-listing at genesis (3 pairs + 3 pools)
