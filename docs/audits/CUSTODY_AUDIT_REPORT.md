...
Treasury signing derives from one treasury master seed. Deposit issuance can optionally derive from a separate deposit master seed while persisting per-deposit seed provenance for backward-compatible sweeps:
```
treasury_root = CUSTODY_MASTER_SEED
deposit_root = CUSTODY_DEPOSIT_MASTER_SEED || treasury_root

deposit_root
    └─ HMAC-SHA256(deposit_root, derivation_path)
        ├─ Solana: Ed25519 (ed25519_dalek::SigningKey::from_bytes)
        └─ EVM: secp256k1 (k256::ecdsa::SigningKey::from_bytes) → Keccak256 → last 20 bytes

persisted deposit record
    └─ deposit_seed_source = treasury_root | deposit_root
...
**Fix applied**: Deduct 5,000 lamports (Solana base fee per signature) from the sweep amount. Native SOL sweep jobs now persist the post-fee `credited_amount` so the wrapped credit matches actual treasury intake. If the balance is dust (≤ fee), the worker keeps the job retriable and waits for additional funds instead of marking it terminally failed.
...
Legacy deposit addresses across both chains were originally deterministically derived from one master seed. The service now supports a separate deposit root for newly issued deposits and persists per-deposit seed provenance so sweeps stay backward compatible. That reduces treasury/deposit coupling for new deposits, but any root compromise still exposes every deposit derived from that root.

**Recommendation**:
- Consider split key custody: master_seed = XOR(key_part_1, key_part_2)
- Keep one part in HSM, one in encrypted config
- Treasury withdrawals now have live Safe-based EVM execution and FROST-based Solana execution; the remaining structural gap is that deposit sweeps still broadcast from locally derived deposit keys, so the seed remains the effective bridge hot-wallet root
...
- `test_collect_and_assemble_threshold_evm_safe_flow` — Safe-owner signature collection and executor relay assembly
- `test_assemble_signed_evm_tx_rejects_mismatched_safe_hash` / `test_assemble_signed_evm_tx_rejects_duplicate_signers` — Safe relay fail-closed validation
- `test_promote_locally_signed_sweep_jobs_*` — honest local-sweep state and audit metadata
- `test_process_sweep_jobs_multi_signer_uses_local_sweep_path` — queued → signing → signed → sweep_submitted worker regression with tx intent cleanup
- `test_process_sweep_jobs_confirmed_enqueues_credit_and_updates_status` — sweep_submitted → sweep_confirmed worker regression with deposit status, reserve, and credit queue side effects
- `test_process_sweep_jobs_multi_signer_without_override_blocks_local_sweep_execution` — queued local sweeps fail closed in multi-signer mode unless the explicit override is enabled
- `test_process_sweep_jobs_reverted_receipt_marks_failed_without_credit` — reverted sweep receipts enter a real failure path without reserve or credit side effects
- `test_process_withdrawal_jobs_burn_*_permanently_fails_without_broadcast` — burn caller, contract, method, and amount mismatches fail closed before any outbound release path
...
| End-to-end deposit flow | CRITICAL | deposit → watcher → sweep → credit lifecycle across chain watchers and sweep confirmation |
| End-to-end withdrawal flow | CRITICAL | withdrawal → burn → sign → broadcast → confirm |

Trust-model note: the report's earlier recommendation to "complete" treasury multisig on Solana is now partly satisfied. Treasury withdrawals have live threshold implementations on Solana and EVM, and newly issued deposits can derive from a dedicated deposit root, but deposit sweeps still remain outside the threshold boundary because broadcast signing is still local.
...
### Critical Fixes
1. **C1** — `broadcast_solana_sweep()`: Deducts 5,000-lamport fee before building transfer, persists the net credited amount for native SOL, and keeps dust balances retriable until additional funds arrive.
