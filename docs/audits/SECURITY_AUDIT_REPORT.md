# Lichen Smart Contract Security Audit Report

**Scope**: All 27 contracts in `/contracts/`  
**Lines audited**: ~42,149 across bountyboard, sporepay, sporepump, sporevault, thalllend, lichencoin, compute_market, dex_core (partial), dex_margin, lichenbridge, prediction_market, lichenid, lusd_token, lichendao, lichenoracle, lichenauction, lichenswap, moss_storage + remaining contracts reviewed for pattern issues.
...
### #2 — CRITICAL | lichencoin | Line ~186 | `approve()` Has No Caller Verification — Anyone Can Set Allowances

**Type**: Missing Caller Verification  
**File**: `contracts/lichencoin/src/lib.rs`
...
### #3 — CRITICAL | lichencoin | Line ~155 | `mint()` Uses Parameter as Caller — Owner Address Can Be Spoofed
...
### #6 — HIGH | sporevault | Line ~430 | `get_user_position` Silent u64 Overflow
...
### #7 — HIGH | sporevault | Line ~370 | `harvest()` u64 Overflow in Fee Calculation
...
### #8 — HIGH | sporepump | Lines ~380–420 | Graduation Partial Cross-Call Failure Not Reverted

**Type**: Unchecked Return Values / Token Accounting Error  
**File**: `contracts/sporepump/src/lib.rs`

**Description**: When a token graduates to the DEX, three cross-contract calls are made (`create_pair`, `create_pool`, `add_liquidity`). Their results are checked only for logging. If `create_pair` succeeds but `add_liquidity` fails, the token is **marked as graduated** (`data[64] = 1`) and the liquidity LICN is permanently locked with no DEX pair. Users can no longer sell, and the tokens are effectively frozen.
...
### #9 — HIGH | lichendao | Lines ~330–400 | `execute_proposal` Does Not Actually Execute Anything

**Type**: Dead Code / Logic Error  
**File**: `contracts/lichendao/src/lib.rs`
...
### #10 — MEDIUM | sporepay | Line ~358 | `create_stream_with_cliff` Missing Reentrancy Guard

**Type**: Missing Reentrancy Guard  
**File**: `contracts/sporepay/src/lib.rs`
...
### #11 — MEDIUM | lichendao | Lines ~470–499 | `cancel_proposal` Missing `get_caller()` Check
...
### #12 — MEDIUM | thalllend | Line ~225 | `withdraw` Blocked During Pause — Funds Can Be Trapped Indefinitely
...
### #13 — MEDIUM | sporepay | Line ~453 | `transfer_stream` Missing Reentrancy Guard
...
### #14 — MEDIUM | lichenoracle | Lines ~150–190 | `submit_price` Accepts Parameter-Provided Feeder Without `get_caller()` Verification

**Type**: Missing Caller Verification  
**File**: `contracts/lichenoracle/src/lib.rs`
...
### #15 — MEDIUM | sporepay | Line ~295 | `cancel_stream` Reports Refund Amount But Makes No Token Transfer
...
### #17 — MEDIUM | thalllend | Line ~370 | `borrow` Collateral Check Uses Plain u64 Multiplication — Overflow Risk
...
| 2 | lichencoin | ~186 | `approve()` — no auth, anyone can set allowances | CRITICAL |
| 3 | lichencoin | ~155 | `mint()` — owner address spoofable via parameter | CRITICAL |
...
| 6 | sporevault | ~430 | `get_user_position` — `shares * total_assets` u64 overflow | HIGH |
| 7 | sporevault | ~370 | `harvest` — `total_assets * allocation` u64 overflow | HIGH |
| 8 | sporepump | ~380 | Graduation partial DEX failure leaves tokens locked | HIGH |
| 9 | lichendao | ~330 | `execute_proposal` marks executed but dispatches no action | HIGH |
| 10 | sporepay | ~358 | `create_stream_with_cliff` — no reentrancy guard | MEDIUM |
| 11 | lichendao | ~470 | `cancel_proposal` — no `get_caller()` check | MEDIUM |
| 12 | thalllend | ~225 | `withdraw` blocked during pause — funds trapped | MEDIUM |
| 13 | sporepay | ~453 | `transfer_stream` — no reentrancy guard | MEDIUM |
| 14 | lichenoracle | ~150 | `submit_price` — feeder address spoofable | MEDIUM |
| 15 | sporepay | ~295 | `cancel_stream` — no actual token transfer | MEDIUM |
| 16 | bountyboard | ~440 | `cancel_bounty` — no actual token transfer | MEDIUM |
| 17 | thalllend | ~370 | `borrow` — collateral check u64 overflow | MEDIUM |
| 18 | lichenauction | ~140 | `create_auction` — no `get_caller()` check | MEDIUM |
| 19 | sporepump | ~244,337,508 | `u64` fns return `200` as error (ambiguous) | LOW |
