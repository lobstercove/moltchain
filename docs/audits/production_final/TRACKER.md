# Lichen — Production Audit Master Tracker
...
|---|---|---|---|---|---|---|
| [x] | `RPC-C01` | 🔴 | `rpc/src/lib.rs:~1300` | Add tier-based rate limiting to `/api/v1/*` DEX REST endpoints — currently unprotected at 5,000 req/s | Copilot | Fixed: added dedicated `/api/v1` tier classification and REST-specific per-tier IP limiter checks in middleware; verified clean compile for `lichen-rpc` |
...
| Status | ID | Sev | File:Line | Task | Owner | Notes |
|---|---|---|---|---|---|---|
| [x] | `RPC-M06` | 🟡 | `core/src/state.rs:6155` | Replace count-offset pagination in `export_cf_page` with cursor-based — O(offset+limit) today | Copilot | Fixed: added cursor-based export pagination in `StateStore` (`export_*_cursor` + cursor-aware `export_cf_page_cursor`) and switched validator snapshot serving path to cursor progression with per-peer/category cursor cache, eliminating repeated O(offset+limit) rescans for sequential chunk requests; compile-validated for `lichen-core` and `lichen-validator` |
...
| [x] | `RPC-L02` | 🟢 | `rpc/src/lib.rs` | Document / implement cross-process IP rate limit sharing | Copilot | Fixed: added opt-in cross-process shared global per-IP limiter via `RPC_RATE_LIMIT_SHARED_FILE` with lock-file coordination and 1s shared counter persistence, while preserving local in-process limiter as fallback; compile-validated via `cargo check -p lichen-rpc` |
| [x] | `RPC-L03` | 🟢 | `rpc/src/ws.rs` | Increase WS broadcast channel capacity from 1000 | Copilot | Fixed: increased base WS event broadcast channel capacity from 1000 to 4096 via `WS_EVENT_CHANNEL_CAPACITY` constant in `WsState::new`; compile-validated via `cargo check -p lichen-rpc` |
...
| [x] | `DEX-C03` | 🔴 | `dex/dex.js` + `rpc/src/dex.rs` | Fix pool price formula: divides by 2^16 instead of 2^32 — prices off by factor ~4.3 billion | Copilot | Fixed: corrected Q32.32 pool price conversion in DEX UI and added RPC-derived `price` field from `sqrt_price / 2^32` squared; compile-validated via `cargo check -p lichen-rpc` |
...
| Status | ID | Sev | File:Line | Task | Owner | Notes |
|---|---|---|---|---|---|---|
| [x] | `DEX-L03` | 🟢 | `contracts/dex_margin/src/lib.rs` (`liquidate`) | Fix liquidator reward skipped when `LICHENCOIN_ADDRESS_KEY` is not configured — reward transfer branch is silently skipped, liquidators receive nothing | Copilot | Fixed: `liquidate` now returns explicit error when reward is owed but `LICHENCOIN_ADDRESS_KEY` is unset, preventing silent reward skips; validated with focused liquidation unit tests + adversarial liquidation test pass (2026-02-27) |
...
| [-] | `T-DEX-012` | Unit | Call `liquidate` with no `LICHENCOIN_ADDRESS_KEY` set | Function returns error; liquidator reward is not silently skipped | Copilot | Not in 2026-02-28 startup-parity run set (matrix+cluster); track in dedicated dex_margin unit pass. |
...
| [x] | `WAL-H03` | 🟠 | `wallet/js/shielded.js:590-615` | Remove plaintext note secrets from localStorage — encrypt before storage | Copilot | Fixed: shielded note persistence now encrypts localStorage payload with AES-GCM using key material derived from spending/viewing keys (`lichen-shielded-storage-v1`) and migrates legacy plaintext records on load; validated via `node tests/test_wallet_audit.js` (`W-22`) |
...
| [x] | `WAL-M02` | 🟡 | `wallet/js/shielded.js:~860` | Add address validation for unshield recipient | Copilot | Fixed: added recipient address validation (`LichenCrypto.isValidAddress`) in both `confirmUnshield` and `unshieldLicn` guard paths to block invalid targets pre-submit; validated via `node tests/test_wallet_audit.js` (`W-16`) |
...
| [x] | `WAL-M08` | 🟡 | `wallet/js/identity.js:~275` | Test `set_rate` WASM encoding against live contract | Copilot | Fixed: added ABI-alignment verification for `set_rate` encoding (`Pubkey + u64`, opcode 41) and call-site spore-unit assertions against LichenID ABI in wallet audit suite; validated via `node tests/test_wallet_audit.js` (`W-17`) |
...
| [x] | `WAL-M11` | 🟡 | `wallet/extension/src/` ws-service.js | Wire WS notifications to popup instead of discarding — popup currently relies on polling only | Copilot | Fixed: WS manager now emits account-change subscription events, background relays as `LICHEN_WS_EVENT`, and popup consumes runtime WS events to refresh balance/activity/shield views; validated via `node tests/test_wallet_extension_audit.js` (`CC-12`) |
...
| [x] | `WAL-L05` | 🟢 | `wallet/js/wallet.js:~1420` | Change explorer links from relative paths to `LICHEN_CONFIG.explorer` base | Copilot | Fixed: wallet activity transaction links now resolve via `LICHEN_CONFIG.explorer` base with encoded signatures and fallback path for local contexts; validated via `node tests/test_wallet_audit.js` (`W-18`) |
| [x] | `WAL-L06` | 🟢 | `wallet/js/crypto.js` | Add note that seed zeroing is best-effort in JS GC environments | Copilot | Fixed: added explicit best-effort zeroing note in `LichenCrypto.signTransaction(...)` to document JS runtime/GC limitations while preserving active buffer wipes (`seed.fill(0)`, `secretKey.fill(0)`) |
| [x] | `WAL-L07` | 🟢 | `wallet/js/wallet.js` + `crypto-service.js` | Consolidate duplicate `keccak256` implementations | Copilot | Fixed: wallet EVM derivation now uses shared `LichenCrypto.generateEVMAddress(...)` path first, eliminating wallet-local keccak dependency drift and keeping a compatibility fallback for older builds |
| [x] | `WAL-L08` | 🟢 | `wallet/index.html` | Replace hardcoded send fee with live `getFeeConfig` RPC call | Copilot | Fixed: send modal fee label is now dynamic (`sendNetworkFeeDisplay`) and wallet send flow fetches `getFeeConfig` (`base_fee_spores`) to update fee display + max-send/fee checks; validated via `node tests/test_wallet_audit.js` (`W-18`) |
...
| [x] | `WAL-L10` | 🟢 | `wallet/extension/src/` provider-router.js | Add expiry to approved origins | Copilot | Fixed: added approved-origin TTL metadata (`lichenApprovedOriginsMeta`, 30-day expiry), prune-on-read cleanup, and expiry refresh on approval while preserving legacy origin list compatibility; validated via `node tests/test_wallet_extension_audit.js` (`CC-17`) |
| [x] | `WAL-L11` | 🟢 | `wallet/extension/src/` popup.js | Replace hardcoded `$0.10` LICN price with live price feed | Copilot | Fixed: popup balance USD display now queries live oracle feed (`/api/v1/oracle/prices`) for `LICN` with short TTL caching and fallback; validated via `node tests/test_wallet_extension_audit.js` (`CC-15`) |
| [x] | `WAL-L12` | 🟢 | `wallet/js/identity.js:~1280` | Verify name auction bid units match contract expectations | Copilot | Fixed: auction bid flow now passes `bid_amount` (LICN units) into `bid_name_auction` args while tx value remains the same bid amount, matching contract expectation that ABI `bid_amount` and transferred value agree in spores; validated via `node tests/test_wallet_audit.js` (`W-21`) |
...
| Status | Test ID | Type | Scenario | Expected | Owner | Notes |
|---|---|---|---|---|---|---|
| [-] | `T-WAL-001` | Integration | Shield 10 LICN end-to-end | Valid Groth16 proof accepted by shielded_pool contract | Copilot | Not in 2026-02-28 startup-parity run set (matrix+cluster); track in dedicated wallet shielded E2E pass. |
...
| [-] | `T-WAL-004` | Integration | Send MAX balance of LICN | TX succeeds (fee reserved automatically) | Copilot | Not in 2026-02-28 startup-parity run set (matrix+cluster); track in dedicated wallet integration pass. |
...
| [x] | `MKT-C03` | 🔴 | `marketplace/js/create.js:~566` | Fix mint opcode binary payload to match `lichenpunks::mint` WASM ABI | Copilot | Fixed: replaced ad-hoc custom mint buffer/opcode with bincode-compatible opcode-7 `MintNftData` payload and system-program mint account routing (`creator, collection, token, owner`) matching chain instruction format; validated via `node tests/test_marketplace_audit.js` (`M-27.10`, `M-27.12`) |
| [x] | `MKT-C04` | 🔴 | `marketplace/js/create.js:~555` | Store metadata via `moss_storage` instead of inline data URI | Copilot | Fixed: removed inline `data:application/json;base64,...` metadata URIs and added MOSS-backed metadata registration (`store_data`) with `moss://<hash>` URI returned to mint flow; validated via `node tests/test_marketplace_audit.js` (`M-27.13`, `M-27.14`, `M-27.15`) |

#### `marketplace/js/item.js` + `profile.js` + `create.js`
| Status | ID | Sev | File:Line | Task | Owner | Notes |
|---|---|---|---|---|---|---|
| [x] | `MKT-C05` | 🔴 | `marketplace/js/item.js`, `profile.js`, `create.js` | Fix all transactions routing to `[0xFF;32]` placeholder — must use deployed marketplace program address | Copilot | Fixed: replaced hardcoded `[0xFF;32]` marketplace routing in item/profile/create with symbol-registry-resolved deployed program IDs (`LICHENMARKET` / `MOSS`) and added audit guards for placeholder removal + resolved program routing; validated via `node tests/test_marketplace_audit.js` (`M-28.*`) |
...
| [x] | `MKT-C07` | 🔴 | All marketplace pages | Wire full auction system (lichenmarket + lichenauction) — currently entirely unwired | Copilot | Fixed: wired auction lifecycle UX and contract calls across item/profile (`create_auction`, `place_bid`, `settle_auction`, `cancel_auction`), added item auction panel state/actions, and integrated auction interaction entrypoints for collected/profile NFTs; validated via `node tests/test_marketplace_audit.js` (`M-38.1`–`M-38.5`) + `cargo check --manifest-path contracts/lichenmarket/Cargo.toml` |
...
| Status | ID | Sev | File:Line | Task | Owner | Notes |
|---|---|---|---|---|---|---|
| [x] | `MKT-H01` | 🟠 | `marketplace/js/create.js` | Fix lichenpunks minting — contract restricts mint to privileged `minter` address, blocking all user mints | Copilot | Fixed: updated `lichenpunks::mint` authorization to allow `minter` or user self-mint (`caller == to`) and hardened create flow with post-mint runtime consistency verification; validated via `node tests/test_marketplace_audit.js` (`M-36.1`, `M-36.4`) + `cargo check --manifest-path contracts/lichenpunks/Cargo.toml` |
...
| Status | ID | Sev | File:Line | Task | Owner | Notes |
|---|---|---|---|---|---|---|
| [x] | `MKT-H09` | 🟠 | All marketplace pages | Add double-fee guard to `lichenmarket::accept_collection_offer` — currently pulls twice from offerer | Copilot | Fixed: `accept_collection_offer` now escrows full payment once in marketplace, refunds escrow on NFT transfer failure, and releases seller proceeds from escrow (no second pull from offerer); validated via `node tests/test_marketplace_audit.js` (`M-37.6`, `M-37.7`) + `cargo check --manifest-path contracts/lichenmarket/Cargo.toml` |
...
| [x] | `MKT-M02` | 🟡 | All marketplace pages | Wire collection-offer system (currently entirely unwired) | Copilot | Fixed: wired collection-offer actions in item/profile (`make_collection_offer`, `cancel_collection_offer`, `accept_collection_offer`) and extended `getMarketOffers` RPC with `include_collection_offers` support; validated via `node tests/test_marketplace_audit.js` (`M-35.1`–`M-35.7`) + `cargo check -p lichen-rpc` |
| [x] | `MKT-M03` | 🟡 | `marketplace-config.js` | Fix `wsUrl: null` for mainnet/testnet — real-time updates disabled in production | Copilot | Fixed: set production WebSocket endpoints for marketplace networks (`wss://ws.lichen.network`, `wss://testnet-ws.lichen.network`) and added audit guards; validated via `node tests/test_marketplace_audit.js` (`M-33.1`, `M-33.2`) |
| [x] | `MKT-M04` | 🟡 | `marketplace/js/profile.js` | Implement Favorites tab — currently permanently stubbed | Copilot | Fixed: implemented wallet-scoped favorites persistence and rendering (`lichenmarket_favorites_v1`) with item-page favorite toggle + profile favorites tab loading/sorting; validated via `node tests/test_marketplace_audit.js` (`M-35.8`–`M-35.10`) |
...
| [x] | `MKT-L07` | 🟢 | `marketplace/create.html` | Align royalty `max="50"` with lichenauction 10% cap | Copilot | Fixed: royalty input max/hint and create-page validation now enforce 0–10%; validated via `node tests/test_marketplace_audit.js` (`M-11.1`, `M-11.2`, `M-11.3`) |
| [x] | `MKT-L08` | 🟢 | `lichenmarket::make_offer` | Add minimum offer floor and per-wallet offer limit | Copilot | Fixed: added `MIN_OFFER_PRICE` floor and `MAX_ACTIVE_OFFERS_PER_WALLET` cap with wallet-scoped active-offer accounting (reserve/release on create/cancel/accept), applied to both `make_offer` and `make_offer_with_expiry`; validated via `node tests/test_marketplace_audit.js` (`M-37.1`–`M-37.5`) + `cargo check --manifest-path contracts/lichenmarket/Cargo.toml` |
| [x] | `MKT-L09` | 🟢 | `lichenmarket::settle_auction` | Handle royalty transfer failure gracefully — seller currently underpaid on failure | Copilot | Fixed: `settle_auction` now handles royalty transfer failure with explicit fallback payout to seller (`royalty_amount`) instead of silently underpaying; validated via `node tests/test_marketplace_audit.js` (`M-38.6`, `M-38.7`) + `cargo check --manifest-path contracts/lichenmarket/Cargo.toml` |
...
| [-] | `T-MKT-004` | Integration | Mint new NFT via create page | Metadata stored via moss_storage, not data URI | Copilot | Not in 2026-02-28 startup-parity run set (matrix+cluster); track in dedicated marketplace integration pass. |
...
| [-] | `T-MKT-007` | E2E | Full auction: list → bid → settle | Correct LICN flows to seller, royalty to creator, fee to treasury | Copilot | Not in 2026-02-28 startup-parity run set (matrix+cluster); track in dedicated marketplace E2E pass. |
...
| [x] | `EXP-M06` | 🟡 | `explorer/js/privacy.js` | Remove local redefinitions of `formatLicnValue`, `formatNumber` etc. — use shared utils | Copilot | Fixed: removed local format/escape/time/copy helpers and switched to shared `formatLicn`, `formatNumber`, `formatTimeFull`, `escapeHtml`, `copyToClipboard` |
...
| [x] | `EXP-L08` | 🟢 | `explorer/js/explorer.js` | Allow RPC `rpc.call()` to carry TypeScript typing | Copilot | Fixed: added generic JSDoc template on `LichenRPC.call()` returning `Promise<T | null>` for TS-aware inference |
...
| Status | ID | Sev | File:Line | Task | Owner | Notes |
|---|---|---|---|---|---|---|
| [x] | `FAU-H01` | 🟠 | `faucet/faucet.js:4` | Change `window.LICHEN_CONFIG` to bare `LICHEN_CONFIG` — `const` never attaches to `window`, production URL never used | Copilot | Fixed: faucet API resolution now reads bare `LICHEN_CONFIG.faucet` first, with safe fallback to legacy `window.LICHEN_CONFIG` |
...
| [x] | `FAU-H07` | 🟠 | `faucet/shared/utils.js:316-322` | Wire `LICHEN_CONFIG.rpc` to `getLichenRpcUrl()` so chain status bar uses correct RPC in production | Copilot | Fixed: `getLichenRpcUrl()` now reads `LICHEN_CONFIG.rpc` (and `window.LICHEN_CONFIG.rpc`) first; faucet shared config now defines dev/prod `rpc` values |
...
| Status | ID | Sev | File:Line | Task | Owner | Notes |
|---|---|---|---|---|---|---|
| [x] | `FAU-M01` | 🟡 | `faucet/faucet.js:78-80` | Replace `../explorer/` relative path with `LICHEN_CONFIG.explorer` base URL | Copilot | Fixed: explorer transaction links now use `LICHEN_CONFIG.explorer` (with safe fallback) instead of hardcoded relative paths |
...
| [x] | `FAU-M04` | 🟡 | `faucet/faucet.js:75` | Change to `data.amount ?? LICN_PER_REQUEST` to prevent "undefined LICN" banner | Copilot | Fixed: success banner and explorer link now use `data.amount ?? LICN_PER_REQUEST` fallback to avoid undefined amount rendering |
...
| Status | ID | Sev | File:Line | Task | Owner | Notes |
|---|---|---|---|---|---|---|
| [x] | `FAU-L01` | 🟢 | `faucet/index.html:28-33` | Replace hardcoded relative nav hrefs with `data-lichen-app` attributes | Copilot | Fixed: main nav and connect-wallet CTA now use `data-lichen-app` routing instead of hardcoded relative hrefs |
...
| Status | ID | Sev | File:Line | Task | Owner | Notes |
|---|---|---|---|---|---|---|
| [x] | `DEV-H01` | 🟠 | `developers/js/developers.js:244-350` | Fix search index — `licn_` prefix on method names returns zero results for all RPC searches | Copilot | Fixed: search now normalizes punctuation and adds RPC alias forms so `licn_*`, `lichen *`, and unprefixed method queries all match reliably |
...
| [x] | `DEV-H06` | 🟠 | `developers/getting-started.html` / `cli-reference.html` | Reconcile `lichen wallet new` vs `lichen wallet create` — one is wrong | Copilot | Fixed: standardized getting-started command to `lichen wallet create` to match CLI reference |
...
| Status | ID | Sev | File:Line | Task | Owner | Notes |
|---|---|---|---|---|---|---|
| [x] | `DEV-M01` | 🟡 | `developers/index.html` | Move `shared-config.js` to load before `developers.js` — `window.lichenConfig` undefined at init | Copilot | Fixed: `shared-config.js` now loads before `js/developers.js` on Developer Hub page |
| [x] | `DEV-M02` | 🟡 | `developers/shared-base-styles.css` / `shared-theme.css` | Resolve dual CSS variable systems — conflicts and undefined variables | Copilot | Fixed: added bidirectional token aliases between base (`--primary` family) and theme (`--teal-*` family) variables to eliminate cross-file style drift and undefined-token risk |
...
| [x] | `DEV-M05` | 🟡 | `developers/sdk-rust.html` WS section | Replace `account.lamports` with `account.spores` | Copilot | Fixed: Rust WS subscription example now references `account.spores` |
| [x] | `DEV-M06` | 🟡 | `developers/lichenid.html` Rust examples | Replace `lichen_client` with `lichen-sdk` | Copilot | Fixed: LichenID Rust examples now import from `lichen_sdk` |
| [x] | `DEV-M07` | 🟡 | `developers/lichenid.html` Python examples | Replace `Client` class with `Connection` class | Copilot | Fixed: LichenID Python examples now use `Connection` class API |
| [x] | `DEV-M08` | 🟡 | `developers/lichenid.html` / `architecture.html` | Standardize Trust tier 1 name — "Known" vs "Verified" inconsistency | Copilot | Fixed: tier-1 label is standardized to `Verified` across LichenID and Architecture docs |
...
| [x] | `DEV-L06` | 🟢 | `developers/contract-reference.html` | Reconcile function count — 227 vs 33 vs 34 across pages | Copilot | Fixed: clarified `227` as all-contract total and standardized LichenID references to `33` functions to remove cross-page mismatch |
...
| [x] | `DEV-L08` | 🟢 | `developers/getting-started.html` | Fix deploy fee description — not "flat 25 LICN", scales with WASM size | Copilot | Fixed: deploy-cost documentation now states size-based pricing instead of a flat-fee model |
...
| [x] | `DEV-L11` | 🟢 | `developers/getting-started.html` | Replace relative path `../faucet/index.html` with `data-lichen-app` link | Copilot | Fixed: getting-started faucet link now uses shared `data-lichen-app="faucet"` routing |
...
| [x] | `WEB-H03` | 🟠 | `website/script.js` / `website/index.html:791-808` | Fix or remove `callContract` example — not present in `LichenRPC` class | Copilot | Fixed: replaced unsupported `callContract` snippet with valid `getBalance` JSON-RPC example |
...
| [x] | `WEB-H05` | 🟠 | `website/index.html:718,1100,1117` | Verify GitHub URL — `https://github.com/lichen/lichen` may not exist | Copilot | Fixed: replaced broken repository links with verified live Lichen GitHub organization URLs |
...
| [x] | `WEB-M04` | 🟡 | `website/shared-theme.css:32-92` | Remove or scope teal/cyan variables unused by website | Copilot | Fixed: removed unused theme tokens (`--teal-dark`, `--blue-primary`, `--blue-accent`) from website shared theme |
...
| [x] | `WEB-M08` | 🟡 | `website/index.html:1052-1090` | Verify / replace placeholder social links (Discord, Twitter, GitHub, Telegram) | Copilot | Fixed: replaced dead/legacy social URLs with verified canonical endpoints (`x.com`, `github.com/lichen`, `t.me/lichen`) and a non-expired Discord destination |
...
Latest matrix evidence: `tests/run-full-matrix-feb24.sh` on external 3-validator cluster (`FORCE_MANAGED_MATRIX_CLUSTER=0`, `LICHEN_RPC=http://127.0.0.1:8899`) completed with `TOTAL=40 PASS=40 FAIL=0` (log: `tests/artifacts/full_matrix_feb24_2026/full-matrix.log`).
...
| [x] | `T-MTX-006` | E2E | Wallet → ZK → Explorer | Shield LICN, unshield LICN | Privacy pool balance correct; no trace in public explorer | Copilot | PASS: `WAL-C01`, `RPC-H07`, and `RPC-H08` closed; shielded wallet + RPC audits pass and end-to-end matrix completed `40/40` |
...
| [x] | `T-REG-004` | Wallet | Send LICN, stake, unstake | Copilot | PASS: executed live `transfer`, `stake add`, and `stake remove` transactions successfully using funded treasury signer |
| [x] | `T-REG-005` | Contracts | Deploy a WASM contract, call a method | Copilot | PASS: deployed `/tmp/licn-contract-test/counter.wasm` and executed live `increment` call with successful signatures |
...
|---|---|---|---|---|---|---|
| [x] | `CON-01` | 🔴 | `contracts/lichenoracle/src/lib.rs:201,828` | Staleness threshold: `3600` → `9_000` slots (1h at 400ms/slot); added `saturating_sub` | Copilot | `get_timestamp()` returns slot number, not seconds. Previous value (3600) caused premature staleness at ~24min. |
...
| [x] | `CON-05` | 🔴 | `contracts/sporepump/src/lib.rs:197` | `transfer_licn_out` now returns `false` when LICN token address unconfigured | Copilot | Was `return true` — sells/withdrawals silently succeeded without actual token transfer |
| [x] | `CON-06` | 🟠 | `contracts/thalllend/src/lib.rs:750` | Health factor calculation cast to `u128` before multiply | Copilot | `deposit * 8500` overflows `u64` for deposits > ~2.17M LICN |
| [x] | `CON-07` | 🔴 | `contracts/lichendao/src/lib.rs:317` | `PROPOSAL_SIZE` corrected from 210 → 212 bytes | Copilot | Actual layout: 5×32 + 6×8 + 4×1 = 212. Undersized constant caused stake refund to skip |
...
| [x] | `GX-03` | 🔴 | `contracts/lichencoin/src/lib.rs:67` | Initial supply changed from 1M → 1B LICN (matching genesis allocation, docs, and README) | Copilot | Was `1_000_000 * 1e9`; now `1_000_000_000_000_000_000` (1B × 10⁹ spores) |
| [x] | `GX-04` | 🟠 | `contracts/lichencoin/src/lib.rs:119` | Documented `mint()` purpose: wrapper-layer only; native LICN supply is protocol-managed from a 500M genesis baseline with epoch-boundary mint settlement | Copilot | `mint()` remains wrapper-scoped; public docs were updated to remove obsolete fixed-supply claims |
...
| [x] | `T-SEC-002` | Unit | `cargo test --workspace` (1085 tests) | 0 failures across all crates | Copilot | Validates contract logic changes (lichencoin, lichendao, thalllend, sporepump, lichenoracle, dex_router, shielded_pool) |
...
| 3 | RPC-02 | Deprecated MossStake methods consume expensive rate budget | Removed `stakeToMossStake`, `unstakeFromMossStake`, `claimUnstakedTokens` from `Expensive` tier | [x] |
| 4 | RPC-03 | Circulating supply ignores staked LICN | Added `total_staked` subtraction: `total_supply - genesis_balance - burned - total_staked` | [x] |
...
| 7 | RPC-06 | Genesis supply label misleading | Clarified `amount_licn` as "original allocation" with updated label | [x] |
...
| 17 | WL-09 | `lichenWalletState` in localStorage | Moved to `sessionStorage` (cleared on tab close) | [x] |
...
| Fixed in code but not tracked | 2 | CON-09 (sporevault alloc cap), CON-11 (compute_market pause) |
