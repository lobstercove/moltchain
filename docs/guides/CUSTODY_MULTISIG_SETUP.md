...
> Production readiness note: the custody binary no longer aborts on multi-signer startup, but threshold support is still chain-specific. Solana treasury withdrawals now use the live two-round FROST coordinator for both native SOL and SPL stablecoins, and EVM withdrawals use Safe-owner signatures plus a coordinator-submitted executor transaction. Deposit sweeps still remain locally signed from derived deposit keys, so new multi-signer deposit issuance and local sweep execution remain hard-disabled until deposit sweeps have a real threshold architecture.

---

## Overview

The custody service currently exposes one single-signer operating mode and one chain-specific multi-signer mode with an explicit local-sweep override boundary:

| Mode | When | How |
|------|------|-----|
| **Single-signer** | 1 signer endpoint configured | One machine holds the active signing key and signs directly |
| **Multi-signer** | 2+ signer endpoints configured | Solana treasury withdrawals use FROST, EVM withdrawals use Safe-owner signatures plus executor submission, and deposit issuance/sweeps fail closed unless the explicit local-sweep override is enabled |
...
**Startup verification for staged multi-signer operation:** The custody service logs on startup:
...
### Solana Signing Flow (Currently Wired)

The live multi-signer Solana withdrawal flow is:
...
The resulting Ed25519 signature is indistinguishable on-chain from a single-signer signature.

For SPL stablecoin withdrawals, custody first ensures the treasury and recipient associated token accounts exist, then signs a single-authority SPL transfer message from the treasury owner through the same FROST aggregation path.

### Remaining gaps

- Deposit sweeps are still signed locally from derived deposit keys rather than through threshold signer coordination. The worker now reflects that honestly by skipping placeholder external sweep-signature collection and promoting those jobs as locally signed before broadcast.
...
```
1. EVM withdrawal requested → custody fetches the Safe nonce and Safe transaction hash for the intended transfer
2. POST /sign to each signer with that Safe transaction hash → collect individual ECDSA signatures
...
6. Coordinator builds a normal executor-owned raw transaction with `to = safe_address` and `data = execTransaction(...)`
7. Broadcast that signed executor transaction to EVM RPC
```
