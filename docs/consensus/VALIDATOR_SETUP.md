...
1. **Transaction-producing slots**:
  - You earn your validator's transaction-fee share when you actually produce blocks
  - Activity improves current-epoch reward projections because more stake-weighted issuance is accruing
  - This is the best signal for near-term operator revenue

2. **Epoch-boundary staking settlement**:
  - The protocol tracks projected per-slot staking rewards throughout the epoch
  - Minting and canonical reward settlement happen only when the epoch boundary executes
  - Explorer and RPC may show projected pending rewards before on-chain settlement finalizes

### Expected Earnings

Do not model validator income as a fixed per-block payout stream. Live earnings depend on:

- Your stake weight and commission settings
- Current validator-set size and uptime
- Transaction-fee share from blocks you actually produce
- The epoch's projected staking reward, which settles only at epoch boundaries
- Whether you are a bootstrap validator still splitting settled rewards between liquid balance and grant recovery

**Costs:**
- Electricity: ~$0.10/day (low-power validator)
- VPS hosting: ~$5-20/month (optional)

**Operator guidance:** Use explorer and RPC reward projections for planning, but treat on-chain settled balances and
total minted supply as canonical only after epoch execution.
...
**Tools:**
- Explorer: http://localhost:3001 (self-hosted)
