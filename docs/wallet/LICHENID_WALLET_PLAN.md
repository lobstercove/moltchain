# LichenID Wallet Integration Plan

**Date:** February 14, 2026
**Status:** Planning
**Branch:** `main`
**Depends on:** LichenID contract (deployed at genesis, 37 exports, 3126 lines)
...
7. [.lichen Name System — Explorer-Wide Integration](#7-licn-name-system--explorer-wide-integration)
8. [RPC Endpoints — New LichenID APIs](#8-rpc-endpoints--new-lichenid-apis)
...
LichenID is Lichen's on-chain identity system — already deployed as a genesis contract with 37 WASM functions, reputation scoring, skill attestations, .lichen naming, and agent discovery. But it has **zero UI integration**. No wallet page shows identity data, no explorer resolves .lichen names, and no RPC endpoints serve identity queries.

This plan connects LichenID to the wallet and explorer, making identities visible, manageable, and central to the Lichen experience.

### What LichenID Already Supports (Contract Level)
...
| .lichen name service | Deployed | `register_name`, `resolve_name`, `reverse_resolve`, `transfer_name`, `renew_name`, `release_name` |
...
| No .lichen name display anywhere | Integrate reverse_resolve across all explorer pages |
...
| No name registration UI | .lichen name search + register flow |
...
- No .lichen name resolution (shows raw base58 addresses everywhere)

### RPC Layer (rpc/src/lib.rs — 6278 lines)

**Existing identity-relevant endpoints:**
- `getProgramStorage` — generic storage dump, can read LichenID keys but requires client-side key construction
- No dedicated LichenID RPC methods exist

**Contracts that already reference LichenID:**
- `dex_governance` — reputation-gated voting (500+ rep required)
- `sporepay` — identity-gated payments (configurable min reputation)
- `dex_rewards` — verified referral bonus rate (15% for LichenID-verified)
...
| **Identity** | LichenID profile, reputation, skills, vouches, achievements, .lichen names | If identity registered |
...
│  │                                                        │  │
│  │  Vouched by (12):                                      │  │
│  │  🟠 bob.lichen  │  🟡 charlie.lichen  │  🟢 dave.lichen    │  │
│  │  🟡 eve.lichen  │  ...8 more                            │  │
│  │                                                        │  │
│  │  Vouched for (5):                                      │  │
│  │  🟢 frank.lichen  │  🔵 grace.lichen  │  ...3 more       │  │
│  │                                                        │  │
...
│  ┌─ .lichen Names ─────────────────────────────────────────┐  │
│  │                                                        │  │
│  │  alice.lichen      Expires: Feb 14, 2027  [Renew]       │  │
│  │  mytrader.lichen   Expires: Aug 01, 2027  [Renew]       │  │
│  │                                                        │  │
...
│  │                                                        │  │
│  │  Endpoint: https://alice-agent.licn.dev/api            │  │
│  │  Availability: 🟢 Available                            │  │
│  │  Rate: 0.5 LICN/request                                │  │
...
```
[Balance: 12,450 LICN] [Token Balance: 3] [Txs: 156] [Account Type: User]
```
...
│                                                                │
│  alice.lichen                                        🟡 Trusted  │
│  LiCN8xK...3nP (Base58)  ·  0x7a3f...b2c1 (EVM)             │
│                                                                │
│         ◉ 12,450.500000000 LICN                               │
│                                                                │
│  ┌──────────────┬──────────────┬──────────────┐               │
...
| Button | Action |
|--------|--------|
| **Send** | Opens modal: recipient (supports .lichen names!), amount, memo |
| **Receive** | Shows QR code with address + .lichen name |
| **Deposit** | Shows bridge instructions (deposit lUSD/WSOL/WETH from external chains) |
| **Identity** | If registered → scrolls to Identity tab. If not → opens registration modal |

### Identity Registration Modal

```
┌─ Create Your LichenID Identity ───────────────────────┐
│                                                       │
...
│                                                       │
│  Optional — Register .lichen Name:                      │
│  [____________].lichen    Duration: [1 year ▼]          │
│                                                       │
...
## 7. .lichen Name System — Explorer-Wide Integration

### Reverse Resolution Everywhere

Every address displayed in the explorer should attempt `.lichen` name resolution:

| Page | Where | Before | After |
|------|-------|--------|-------|
| address.html | Page title | `Address: LiCN8xK...3nP` | `alice.lichen (LiCN8xK...3nP)` |
| transaction.html | From/To fields | `LiCN8xK...3nP` | `alice.licn` with hover for full address |
| transactions.html | Table rows | Raw addresses | .lichen names where available |
| block.html | Producer/Validator | Raw address | .lichen name badge |
| validators.html | Validator list | Raw addresses | .lichen names |
| contracts.html | Deployer | Raw address | .lichen name |

### Implementation Approach

1. **Batch resolution** — New RPC endpoint `batchReverseLichenNames([addr1, addr2, ...])` returns a map of address→name
2. **Client-side cache** — `address.js` / `utils.js` maintain a `Map<string, string|null>` of resolved names
3. **Display helper** — `formatAddressWithName(address, name)` renders either `name.licn` with tooltip or truncated address as fallback
...
┌─ .lichen Name Service ─────────────────────────────────────────┐
│                                                               │
│  Search: [______________].lichen  [Check Availability]         │
│                                                               │
...
│                                                               │
│  [Register trading_bot.licn]                                  │
│                                                               │
│  ── Recently Registered ─────────────────────                │
│  alice.lichen       │ Expires: 2027-02-14                      │
│  bob.lichen         │ Expires: 2027-08-01                      │
│  oracle_node.lichen │ Expires: 2028-02-14                      │
└───────────────────────────────────────────────────────────────┘
```
...
## 8. RPC Endpoints — New LichenID APIs

### New Endpoints

| Method | Params | Returns | Description |
|--------|--------|---------|-------------|
| `getLichenIdIdentity` | `[pubkey]` | Identity record (JSON) | Get full identity for an address |
| `getLichenIdReputation` | `[pubkey]` | `{ score, tier, tier_name }` | Quick reputation lookup |
| `getLichenIdSkills` | `[pubkey]` | Array of skills with attestation counts | List all skills |
| `getLichenIdVouches` | `[pubkey]` | `{ received: [...], given: [...] }` | Vouch network |
| `getLichenIdAchievements` | `[pubkey]` | Array of achievements with timestamps | Achievement list |
| `getLichenIdProfile` | `[pubkey]` | Complete profile (identity + skills + vouches + endpoint + metadata) | Composite query |
| `resolveLichenName` | `[name]` | `{ owner, registered_slot, expiry_slot }` | Forward name lookup |
| `reverseLichenName` | `[pubkey]` | `{ name }` or null | Reverse name lookup |
| `batchReverseLichenNames` | `[pubkey1, pubkey2, ...]` | `{ addr1: name1, addr2: null, ... }` | Batch reverse for UI |
| `searchLichenNames` | `[prefix]` | Array of matching names | Name search |
| `getLichenIdAgentDirectory` | `[{ type?, available?, limit? }]` | Array of agent profiles | Agent discovery |
| `getLichenIdStats` | `[]` | `{ total_identities, total_names, tier_distribution }` | Global stats |

### Implementation Pattern

Each endpoint reads from LichenID contract storage using key patterns:

```rust
async fn handle_get_lichenid_identity(state: &RpcState, params: ...) -> Result<Value, RpcError> {
    let pubkey = parse_pubkey(params)?;
    
    // Derive LichenID contract address
    let lichenid_addr = state.get_symbol_address("YID")?;
    
    // Read identity record: key = "id:{hex(pubkey)}"
    let key = format!("id:{}", hex::encode(pubkey.0));
    let contract_account = state.get_contract_storage(&lichenid_addr, &key)?;
    ...
    let reputation = state.read_storage_u64(&lichenid_addr, &rep_key)?;
    
    // Read .lichen name: key = "name_rev:{hex(pubkey)}"
    let name_key = format!("name_rev:{}", hex::encode(pubkey.0));
    let licn_name = state.read_storage_string(&lichenid_addr, &name_key)?;
...
| .lichen name (fwd) | `name:{name_bytes}` | 48 bytes → owner + reg_slot + expiry |
| .lichen name (rev) | `name_rev:{hex32}` | Variable → name bytes |
...
│  ┌─ Agent Card ───────────────────────────────────────────┐   │
│  │  📈 alice.lichen                           🟡 Trusted     │   │
│  │  Type: Trading  │  Rep: 742  │  Rate: 0.5 LICN/req    │   │
│  │  Availability: 🟢 Available                             │   │
│  │  Endpoint: https://alice-agent.licn.dev/api             │   │
...
│                                                                │
│  ┌─ Agent Card ───────────────────────────────────────────┐   │
│  │  🔮 price_oracle.lichen                    🟠 Established  │   │
│  │  Type: Oracle  │  Rep: 2,340  │  Rate: 0.1 LICN/req   │   │
│  │  Availability: 🟢 Available                             │   │
│  │  Endpoint: https://oracle.lichen.network/v1               │   │
...
Reputation unlocks features across Lichen:
...
| 1,000+ | SporePay identity-gated payments unlocked |
...
```
┌────────────────────────────┐
│ 🟡 alice.lichen  Rep: 742   │
│ [████████░░] Trusted       │
└────────────────────────────┘
```

Small variant (inline): `🟡 alice.lichen (742)`
...
```
         eve.lichen (🟢)
            │
    bob.lichen (🟠) ──── alice.lichen (🟡) ──── frank.lichen (🟢)
            │                │
   charlie.lichen (🟡)    dave.lichen (🟢)
```
...
### Where LichenID Appears Across the Platform

| Feature | Integration Point |
|---------|------------------|
| **Explorer — addresses** | .lichen name resolution, trust tier badge next to every address |
| **Explorer — transactions** | Show .lichen names in from/to columns |
| **Explorer — validators** | Show .lichen name and tier for each validator |
| **Explorer — contracts** | Show deployer's .lichen identity |
| **Wallet — balance card** | .lichen name as primary identifier, identity button |
| **Wallet — send modal** | Accept .lichen names as recipient (auto-resolve) |
| **DEX — trading** | Show trader .lichen name and tier in order book |
...
| **SporePump** | Creator identity badge on launchpad tokens |
| **SporePay** | Identity-gated payment streams |
| **Agent directory** | Full discovery interface |

### .lichen Name Input Component

Reusable across all forms:

```
[alice.lichen                    ] ← resolves → LiCN8xK...3nP
                                  ✅ resolved to alice.lichen (🟡 Trusted)
```

If input looks like a .lichen name → resolve via RPC
...
```
┌─────────────────┐     ┌─────────────────┐     ┌──────────────┐
│  LichenID WASM   │────►│  RPC Layer      │────►│  Explorer UI │
...
| Register .lichen name | `register_name(caller, name, name_len, duration_years)` | Contract call tx |
...
| Set rate | `set_rate(caller, licn_per_unit)` | Contract call tx |
...
const lichenNameCache = new Map(); // address → { name, fetchedAt }
...
```
A.1  Implement getLichenIdIdentity RPC endpoint
A.2  Implement getLichenIdReputation RPC endpoint
A.3  Implement reverseLichenName + batchReverseLichenNames
A.4  Implement resolveLichenName
A.5  Implement getLichenIdProfile (composite query)
A.6  Implement getLichenIdSkills, getLichenIdVouches, getLichenIdAchievements
A.7  Implement getLichenIdAgentDirectory
A.8  Implement getLichenIdStats
...
B.3  Enhance balance card: .lichen name display, tier badge, action buttons
...
C.7  .lichen names section: owned names with expiry dates, renew/transfer actions
...
D.2  .lichen name registration: search, check availability, register
D.3  Send modal with .lichen name resolution
...
### Phase E — Explorer-Wide .lichen Integration

```
E.1  Transaction page: resolve .lichen names in from/to
E.2  Transaction list: batch resolve all addresses
E.3  Block page: resolve producer/validator names
E.4  Validator page: show .lichen names + trust tiers
E.5  Contract page: show deployer .lichen name
E.6  Search bar: support .lichen name search (type "alice.lichen" → navigate to address)
```
...
G.4  Name auction: premium .lichen names sold via auction
```
...
test_resolve_licn_name
test_resolve_nonexistent_name
test_reverse_licn_name
...
□ Balance card shows .lichen name if registered
...
□ .lichen names section shows owned names with dates
□ "Create Identity" modal opens and submits transaction
□ .lichen name search resolves availability
□ Send modal accepts .lichen names and resolves them
□ Vouch button sends transaction
□ Transaction page shows .lichen names in from/to
□ Validator page shows .lichen names
□ Search bar resolves .lichen names to addresses
...
test_licn_name_in_send_transaction
...
|----------|---------|----------------|
| Identity registration cost? | Free (gas only), Small fee (1 LICN), Significant fee (10 LICN) | **Free (gas only)** — maximize adoption, reputation gates protect quality |
| .lichen name pricing? | Flat fee per year, Length-based (short=expensive), Auction | **Length-based** — 3-char: 500 LICN ($50), 4-char: 100 LICN ($10), 5+: 20 LICN ($2) per year |
| Should .lichen names expire? | Yes (annual renewal), No (perpetual), Grace period | **Yes with grace** — 1yr registration, 30-day grace period before release |
...
*End of LichenID Wallet Integration Plan*
