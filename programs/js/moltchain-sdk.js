/**
 * MoltChain SDK - Production JavaScript Client
 * Complete SDK for interacting with MoltChain blockchain
 * 
 * Features:
...
const MOLTCHAIN_CONFIG = {
    networks: {
        ...
        testnet: {
            ...
            compiler: 'https://testnet-rpc.moltchain.network/compile',
            chainId: 2
        },
        local: {
    ...
    units: {
        ...
        WEI_PER_MOLT: 1_000_000_000_000_000_000n
    }
};
...
class MoltChainRPC {
    constructor(network = 'testnet') {
        ...
        this.config = MOLTCHAIN_CONFIG.networks[network];
        if (!this.config) {
    ...
    async stakeToReefStake(params) {
        return await this.call('stakeToReefStake', [params]);
    }

    async unstakeFromReefStake(params) {
        return await this.call('unstakeFromReefStake', [params]);
    }
    ...
    async getReefStakePoolInfo() {
        return await this.call('getReefStakePoolInfo');
    }
...
class MoltChainWallet {
    ...
    static import(json, password) {
        ...
        return new MoltChainWallet(seed);
    }
...
class ProgramDeployer {
    ...
    async deploy(wasmBytes, options = {}) {
        const {
            initialFunding = 1_000_000_000, // 1 MOLT
            verify = false,
            metadata = {},
            initData = null,
...
if (typeof window !== 'undefined') {
    window.MoltChain = {
        RPC: MoltChainRPC,
        WebSocket: MoltChainWS,
        Wallet: MoltChainWallet,
        TransactionBuilder,
        ProgramDeployer,
        CONFIG: MOLTCHAIN_CONFIG,
        utils: {
            base58Encode,
            base58Decode,
            bytesToHex,
    ...
    console.log('🦞 MoltChain SDK loaded');
}
