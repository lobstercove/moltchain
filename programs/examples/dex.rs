...
/// Get pool info
#[no_mangle]
pub extern "C" fn get_pool_info(pool_id: [u8; 32]) -> Result<Pool, String> {
...
#[no_mangle]
pub extern "C" fn get_liquidity_balance(
    owner: [u8; 32],
    pool_id: [u8; 32],
) -> Result<u64, String> {
    let position = get_storage::<LiquidityPosition>(&position_key(&owner, &pool_id))?
...
/// Flash loan borrow
#[no_mangle]
pub extern "C" fn flash_loan_borrow(
    pool_id: [u8; 32],
    amount: u64,
    token_is_a: bool,
) -> Result<u64, String> {
    ...
    let reserve = if token_is_a { pool.reserve_a } else { pool.reserve_b };
    if amount > reserve {
    ...
    emit_event("FlashLoan", &format!("Borrowed {} (repay: {})", amount, repay_amount));
